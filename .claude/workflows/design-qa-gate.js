...
const lenses = [
  ...
  { key: 'room-access-leak', q: `LENS — ROOM ACCESS LEAK (blocking, Tier-1). SCOPE GATE FIRST: this lens applies ONLY to the cohort-rooms phases R0, R1, R2, R3, R4. Phase under review is "${PHASE}" — if that is not one of those, return verdict "pass" with zero findings and summary "not a rooms phase" and do NOTHING else. Otherwise: the adversarial access suite is the sign-off artifact that proves one cohort's private content cannot leak to another cohort's students, and it must be re-proven on every rooms phase because R1–R4 add UI and RPC surface on top of the R0 wall. PREREQUISITE — DO THIS BEFORE THE SUITE COMMAND, EVERY RUN, ON A FRESHLY BUILT SHADOW. The suite refuses to run against a shadow that lacks production's table grants, and exits 2 saying so. That is correct behaviour and a SETUP STEP, not a finding: a database built from supabase/migrations/ alone grants the client roles almost nothing, PostgreSQL checks the GRANT before it ever consults RLS, and every RLS assertion would pass vacuously. So provision the shadow first: \`psql "$SHADOW_DB_URL" -f qa-harness/shadow-grants.sql\` BEFORE \`supabase db push\` (that pass arms the default privileges, which only affect tables created afterwards), then \`db push\`, then run the same psql line AGAIN (that pass applies the generated per-table grants). Both passes are idempotent; the ORDERING block at the top of qa-harness/shadow-grants.sql is the authority. If the shadow was CLONED from prod rather than rebuilt, it already carries the grants and both passes are no-ops — run them anyway. Ask the orchestrator for SHADOW_DB_URL if it is not in the environment. Only report a grant-precondition abort as a finding if it persists AFTER both passes have been run — then it is critical, because it means the shadow cannot be provisioned and the wall cannot be proven.
