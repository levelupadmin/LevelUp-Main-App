...
| **V9** | "A resume position survives an app restart" | **There is no in-app player anywhere in this repo.** `MySessionsPage` renders `recording_url` as a plain `<a href>`; grep finds no embed. Resume is unachievable without building a player, which is not in scope. Record `completed` on open, render progress from existing rows, and DROP the resume claim rather than faking it. |
