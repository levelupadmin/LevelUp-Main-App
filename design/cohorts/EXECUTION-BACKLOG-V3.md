...
1. **The payment pipeline is sacred and untouched.** The staged checkout (`create-razorpay-order` staged branch, `verify-razorpay-payment`, `razorpay-webhook`) and the `ApplicationStatus.tsx`'s `isIOS()` guards `isIOS()` staged-payment revenue guard are **do-not-touch** (PRD §4.4 / NFR-SEC-5). This program **reads and adds beside** them; it never modifies them. Any task whose diff touches these files or that guard is a bug in the task. Verify `grep -n "isIOS" src/pages/ApplicationStatus.tsx` diff = 0 after every funnel phase.
...
**Files:** `src/lib/flags.ts` *(new — the single flag registry; see Rollout §5)*, `src/hooks/useFunnelStage.ts` *(new)*, `src/pages/ApplicationStatus.tsx` *(READ-ONLY consumer — do NOT touch the `isIOS()` guard)*
...
- **⟢ CONCRETE PILOT BINDING (Rahul, 2026-07-22).** The abstract "next intake window" is now a real row: **`Creator Academy Edition 2`** (offering `449056b9-9269-4bc5-ba8b-4c079c2104ee`, slug `creator-academy-edition-2`, `payment_mode='staged'`, `draft`+non-public, wired to Tally form `81dRPA`) **is the funnel pilot — intake is OPEN on Edition 2.** **`Creator Academy Edition 1` intake is CLOSED and that batch is ~4 weeks into delivery; Rahul RULED it is NOT wired into the app at all** — it finishes on WhatsApp and is never migrated mid-flight (this is RUNNING-BATCH-1 applied to a real batch), specifically so the redesigned room isn't first seen as the old `/cohort` dashboard. **Consequence to plan around: the room (Slice 3) now has a real business deadline — Edition 2's students convert within weeks of intake opening and will be the FIRST cohort born in the room, so R0→R1 must land before Edition 2's cohort start date.**
