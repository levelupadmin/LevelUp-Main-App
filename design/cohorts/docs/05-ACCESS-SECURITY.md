...
| **`accepted`** *(MEMBER-1 2026-07-18)* | Authenticated, `status='accepted'`, **confirmation-fee unpaid** | Own row + funnel stage; the **marketing-class confirm-seat veil** (public-safe `offerings` data only) | Reading **any** room-content table; any read into room content at all — **there is no preview RPC** and no membership row (zero room read grant); reading the `cohort_room_configs` row |
| **`pre_member_A1`** *(MEMBER-1 2026-07-18)* | Authenticated, `confirmation_paid`, scoped **`pre_member`** in offering A batch A1 | **Only** the whitelisted pre_start subset (masthead/theme, this-week overview = session titles+dates, cohort-mate presence/count, announcements READ, upcoming-session calendar) | Full curriculum detail, **recordings**, assignments/submissions, **feedback**, mentor/instructor materials; **posting/writing in community** (read-only); anything full `member` sees |
...
Membership rows in `cohort_room_members` are stamped with a `source` and `role`. Only the server writes them — `authenticated` gets **SELECT-own and no write verb** (`0002_cohort_room_members.sql:190-195`; the exact per-verb grant end state, including `TRUNCATE`, is §7.1).
...
flowchart TD
    ...
    AP -.->|confirmation_paid → pre_member (MEMBER-1; accepted writes NO row) — NOT DRAFTED, R-D2 OFF| R
...
| 3 | **Application status (pre-start lobby)** | **NOT DRAFTED.** Per **MEMBER-1 (ruled 2026-07-18)** the grant gate is **`confirmation_paid` ONLY** — the INSERT fires on the `confirmation_payment_id` stamp, and **`accepted` writes NO row** (accepted = veil-only, no room read grant; supersedes the pre-MEMBER-1 draft *comment* at `0002:74-77` which spanned `accepted/confirmation_paid/balance_paid`). The draft has only the `pre_member` value in the CHECK constraint (`0002:19`) — no `cohort_applications` trigger and no `pre_member` INSERT exist yet. Enabling it is net-new (see LOBBY-1 / SEC-MEMBER-1) | `pre_member` | **OFF (R-D2) — unbuilt; MEMBER-1 target = confirmation_paid** | `0002:19` (CHECK only); MEMBER-1; SEC-MEMBER-1; PRD R-D2 |
...
> - Behavior: `cohort_room_members` is written only by the resolver triggers, the alumni-flip trigger, the nightly reconcile, and the admin-only manual-grant RPC. `authenticated` holds `SELECT` on its own rows and no `INSERT/UPDATE/DELETE/TRUNCATE` — all four `REVOKE`d explicitly, because the platform bootstrap grants them by default, so the REVOKE is mandatory, not implied. **`TRUNCATE` is in that list and `REFERENCES`/`TRIGGER` deliberately are not: §7.1 carries the authoritative per-verb statement** (the shorthand "`SELECT` and nothing else" is false on a bootstrapped database).
...
stateDiagram-v2
    ...
    interview_done --> accepted: TeleCRM decision (mirrored)
    interview_done --> waitlisted: TeleCRM decision (mirrored)
    interview_done --> rejected: TeleCRM decision (mirrored)
    ...
    note right of accepted
        accepted = NO room grant (MEMBER-1 2026-07-18):
        marketing-class veil only, zero room read,
        no preview RPC. pre_member starts at confirmation_paid.
        accepted is TeleCRM-SOURCED (SOR-1 2026-07-18):
        the app READS/mirrors it, never writes a funnel status.
    end note
...
| `interview_scheduled` / `interview_done` | Applicant in review | Own row; own interview modality/slot | The decision (until TeleCRM records it and the app mirrors it — SOR-1; the app never writes it); any room | none | PRD §5.5 |
| `accepted` (not yet paid) | Admitted, confirmation-fee unpaid | Own row; the **LOCKED FUTURE VIEW veil** on the confirm-seat screen — built **only** from public-safe / marketing-class data (the `offerings` row: title, poster, publicly-advertised faculty names, static module-name lock chips), the same class the public offering page exposes. **No preview RPC; no read into any room-content table** *(MEMBER-1 2026-07-18: the earlier `get_cohort_room_preview` path is DELETED)* | **Any** room-content table (announcements, feed/replies, resources, demo, recordings), `cohort_weeks`/`live_sessions`, the `cohort_room_configs` row, roster PII, fees, every other applicant's row — `accepted` holds **zero room read grant** | **none** (no membership row; marketing-class veil only) | PRD §5.7 REQ-LOCK-1; MEMBER-1; SEC-MEMBER-1 |
| `confirmation_paid` / `balance_paid` | Paying in (seat confirmed) | Own row; the unlock animation; **enters the room as a scoped `pre_member`** — the whitelisted pre_start subset (masthead/theme, this-week OVERVIEW = session titles + dates only, cohort-mate presence/count, welcome/announcements channel READ-only, upcoming-session schedule/calendar) *(RAHUL DECISION line items, SEC-MEMBER-1)* | Full curriculum detail, **recordings**, assignment content/submission, **feedback**, mentor/instructor materials; **community write (read-only)** — all unlock at `enrolled`; any other room | **`pre_member`** (MEMBER-1; resolver-written on the `confirmation_payment_id` stamp) — its **own** access level, `member` NOT widened to include it | PRD §5.7 REQ-LOCK-2; MEMBER-1; SEC-MEMBER-1 |
...
**The crew must not implement the veil by loosening any content, week, or config RLS policy.** It is built **only** from the `offerings` row + static chrome — never by widening a table policy and *(MEMBER-1 2026-07-18)* **never by a preview RPC into room content** (the `get_cohort_room_preview` path is deleted; `accepted` holds zero room read grant — SEC-MEMBER-1).

> **⚠️ MEMBER-1 RULING — RAHUL, 2026-07-18 (authoritative; supersedes the earlier central-resolution framing this block previously carried).** *[Margin note, 2026-07-18: this block previously read that MEMBER-1 adopted VEIL-SOURCE-1 **option (b)** — a whitelisted `get_cohort_room_preview` RPC for `accepted`. Rahul has now RULED against that. The preview-RPC path is **DELETED**; `accepted` holds **zero room read grant.**]* The resolved three-tier boundary is: **(1) `accepted`** (post-interview, confirmation-fee unpaid) → **NO membership row, NO read path into any real room-content table, NO preview RPC into room content.** The only room-adjacent surface is the **LOCKED FUTURE VIEW veil** on the confirm-seat screen — a teaser built **only** from public-safe / marketing-class data (the same class the public offering page already exposes), **never** real room rows. This is VEIL-SOURCE-1's offering-chrome veil (needs no new read surface, loosens no RLS). **(2) `confirmation_paid`** → LOBBY-1 turned **ON** as a scoped `pre_member` row (not `accepted`), entering the room as a LIMITED, heavily-redacted member (whitelist in SEC-MEMBER-1). **(3) `enrolled`** → full `member`, nothing redacted, full read + community write. Rationale (record it): smaller attack surface — `accepted` users carry no RLS grant into room content at all — and it matches Rahul's flow-feedback "locked future view → excitement to confirm unlocks it," extended to a two-stage ladder (locked marketing teaser → confirm → scoped-redacted `pre_member` → remaining fee → full `member`). Security guarantees still bind (membership server-derived, no RLS loosening, `member` never widened to include `pre_member`). **See SEC-MEMBER-1 (§5.4) for the authoritative, resolved statement.** LOBBY-1's "OFF at `accepted`" is now the ruling itself (not a conservative alternative): `accepted` never gets a lobby.

> **VEIL-SOURCE-1 — RULED (Rahul, 2026-07-18): the veil shows marketing-class data only.** *[Margin note, 2026-07-18: previously a RAHUL DECISION with recommended default "offering-chrome veil." Rahul's MEMBER-1 ruling adopts exactly that and forecloses the two net-new paths below.]* An `accepted`-but-unpaid applicant **cannot** legally read week/session/config rows under the current (correct) RLS, and the ruling declines to build any non-member-readable projection that would carry them. So the confirm-seat veil is built **only** from public-safe / marketing-class data — the `offerings` row (title, poster, publicly-advertised faculty names) plus static module-name lock chips — the same class the public offering page already exposes, **never real room rows.** The two previously-tabled lawful-but-net-new paths are both **rejected** for `accepted`: neither **(a)** a LOBBY at `accepted` (LOBBY-1 now starts at `confirmation_paid`, not `accepted`) nor **(b)** a dedicated offering-level preview projection / `get_cohort_room_preview` RPC. The offering-chrome veil needs no new read surface and no RLS change; it is the ruled default precisely because it does not let the veil become a reason to loosen the wall.

> **SEC-STATE-1 — Access follows D2 state exactly; `accepted != member` and `accepted` holds ZERO room read grant.** `🔴 Tier 1`
> *[Margin note, 2026-07-18 — MEMBER-1: this requirement is unchanged in substance for `accepted` but is now the hardened boundary. `accepted` gets no room-content read **and no preview RPC** (the deleted `get_cohort_room_preview` path). The `pre_member` tier that MEMBER-1 turns ON lives at `confirmation_paid`, not `accepted` — see SEC-MEMBER-1.]*
> - Behavior: Only `enrolled` + batch assignment (or an admin/manual grant) yields full room-content access; `confirmation_paid` yields the scoped, redacted `pre_member` subset (SEC-MEMBER-1). **`accepted` grants no room-content read at all** — no content-table read, **no preview RPC**. The veil is rendered from the `offerings` row the applicant already owns plus static module chrome — **never** from content tables, **never** from `cohort_weeks`/`live_sessions`, and **never** from the member-gated `cohort_room_configs` row (SEC-CFG-1, VEIL-SOURCE-1).
> - Acceptance: An `accepted` (unpaid) fixture reading `cohort_announcements`/`cohort_room_posts`/`cohort_room_post_replies`/`cohort_resources`/`cohort_demo_entries`/`cohort_weeks`/`live_sessions`/`cohort_room_configs` for the target offering gets **zero rows / denied**, and there is **no `get_cohort_room_preview` RPC to call**; the veiled UI still renders from the offering row + static chrome alone (assert the client issues no read against any room-content surface for a non-member). §7 R10.
> - Note on the `pre_member` tier (SEC-MEMBER-1): turning it ON at `confirmation_paid` is **net-new work, not a flag flip.** The draft has only the `pre_member` CHECK value (`0002:19`) and a descriptive comment (`0002:74-77`) — there is no `pre_member` INSERT in the resolver body (`0002:82-111`) and no `cohort_applications` trigger anywhere. Enabling it requires a new resolver branch, a new `AFTER UPDATE OF status ON cohort_applications` trigger keyed on the `confirmation_payment_id` stamp, and a separate `cohort_room_in_lobby()` helper.
> - Source: PRD §5.7 REQ-LOCK-1/2, R-D2; `0001:79-81`; `20260526180000:322-330`; `0002:19,74-77,82-111`; `ROOMS-ARCHITECTURE.md` §6.1.

> **LOBBY-1 — RULED (Rahul, 2026-07-18): pre-start lobby is ON at `confirmation_paid`, and ONLY there.** *[Margin note, 2026-07-18: previously a RAHUL DECISION with recommended default "OFF — enrolled-only." Rahul's MEMBER-1 ruling turns the lobby ON, but the entry gate is `confirmation_paid` (the scoped `pre_member` row), NOT `accepted`. `accepted` never enters a lobby — it holds zero room read grant and sees only the marketing-class veil (VEIL-SOURCE-1, SEC-MEMBER-1).]* Turning it on is **net-new work, not a flag flip** — `0002:74-77` is a *comment*, not a code branch; the resolver body (`0002:82-111`) has no `pre_member` INSERT and there is no `cohort_applications` trigger in the draft. The build is: (1) a new resolver branch that INSERTs a `pre_member` row on the **`confirmation_payment_id` stamp** (confirmation-fee paid), (2) a new `AFTER UPDATE OF status ON public.cohort_applications` trigger to fire it (exception-guarded per SEC-ENT-2), and (3) a separate `cohort_room_in_lobby()` helper. The lobby is its **own** access level — `cohort_room_is_member` (full member) stays enrolled-only; `cohort_room_in_lobby()` gates only the SEC-MEMBER-1 redaction whitelist (masthead/theme, this-week overview = session titles+dates, cohort-mate presence/count, announcements READ, upcoming-session calendar) and **never** the recordings shelf, full curriculum, assignments, feedback, mentor materials, or any community **write**. Do **not** widen `member` to include `pre_member`. The `pre_start` *phase* for already-enrolled members is a different thing (an enrolled member before weeks begin), not the lobby.
...
> **SEC-MEMBER-1 — The MEMBER-1 three-tier access boundary: marketing-class veil for `accepted` (zero room read); scoped `pre_member` at `confirmation_paid`; full `member` at `enrolled`.** `🔴 Tier 1`
> *[Margin note, 2026-07-18 — RULED by Rahul. This RE-RULES the prior framing of this requirement, which served `accepted` via a "redacted SECURITY DEFINER preview RPC into room content" (the council's `get_cohort_room_preview` proposal, VEIL-SOURCE-1 option (b)). **That preview-RPC path is DELETED.** `accepted` now holds **zero read grant into any room-content surface** — no membership row, and no preview RPC into room content of any kind. Rationale (recorded): smaller attack surface — `accepted` users carry no RLS grant into room content at all — and it matches Rahul's flow-feedback "locked future view → excitement to confirm unlocks it," extended to a two-stage ladder (locked marketing teaser → confirm → scoped-redacted `pre_member` → remaining fee → full `member`).]*
> This is the access-doc face of the centrally-resolved **MEMBER-1** decision (canonical: `02-STATE-MACHINE.md` §3.4 / `03-DATA-MODEL-ERD.md` §4.6a; PRD REQ-LOCK-1/2). It supersedes this doc's earlier framing in the §4 table, VEIL-SOURCE-1 and LOBBY-1. The three tiers:
>
> **Tier 1 — `accepted` (post-interview, confirmation-fee unpaid): NO room read grant.** No `cohort_room_members` row; **no read path into any real room-content table**; **no preview RPC into room content** (the `get_cohort_room_preview` path is deleted — see the margin note). The only room-adjacent surface is the **LOCKED FUTURE VIEW** veil on the confirm-seat screen: a teaser built **only** from public-safe / marketing-class data — the same class the public offering page already exposes (the `offerings` row: title, poster, publicly-advertised faculty names, static module-name lock chips) — **never real room rows** (no `cohort_weeks`, no `live_sessions`, no `cohort_room_configs`, no content tables). The confirm CTA is present. This is exactly VEIL-SOURCE-1's offering-chrome veil: it needs no new room read surface and loosens no RLS.
>
> **Tier 2 — `confirmation_paid` / `balance_paid`: scoped `pre_member`, heavy redaction.** The resolver writes a `pre_member` `cohort_room_members` row on the confirmation-fee payment stamp (a new `AFTER UPDATE OF status ON cohort_applications` branch keyed on `confirmation_payment_id`, exception-guarded per SEC-ENT-2). `pre_member` **enters the room as a LIMITED member** gated by a dedicated `cohort_room_in_lobby()` helper — it is its **own** access level, NOT `member`. **`cohort_room_is_member` (full member) is NOT widened to include `pre_member`.**
>   > **RAHUL DECISION — the `pre_member` redaction whitelist (recommended default; the specific line items are Rahul's to tune).**
>   > - **`pre_member` CAN read:** the room masthead / theme, a this-week **OVERVIEW** (session **titles + dates only**), cohort-mates **presence / count**, the welcome / announcements channel (**READ only**), and the upcoming-session **schedule / calendar**.
>   > - **`pre_member` CANNOT read or do:** full curriculum detail, **any recordings**, assignment content or submission, **any feedback**, mentor / instructor materials, and **CANNOT post or write in community (read-only)**. Community **write** and full content unlock at `enrolled`.
>
> **Tier 3 — `enrolled` (remaining / full fee) + batch: full `member`.** Nothing redacted; full read + community **write**, scoped to their offering+batch (ROSTER-SCOPE-1). Membership is server-derived, never client-claimed (SEC-ENT-1 holds).
>
> - Behavior invariants (bind under all three tiers): membership is server-derived, never client-claimed (SEC-ENT-1); the veil loosens no content/week/config RLS (SEC-STATE-1 / SEC-CFG-1); `member` is never widened to include `pre_member`.
> - Acceptance: (a) an `accepted` fixture reading **any** room-content table (`cohort_announcements`/`cohort_room_posts`/`cohort_room_post_replies`/`cohort_resources`/`cohort_demo_entries`/`cohort_weeks`/`live_sessions`/`cohort_room_configs`) for the target offering gets **zero rows / denied**, and there is **no `get_cohort_room_preview` RPC to call** (§7 R10); (b) a `pre_member` fixture reads only the whitelisted pre_start subset and gets **zero rows / denied** on every redacted-out surface — full curriculum, recordings, assignments, feedback, mentor materials, and any community **write** (post/reply) — until `enrolled` (§7 R11); (c) a raw client INSERT of a `pre_member` row is rejected (SEC-ENT-1). Per `CLAUDE.md` Tier-1, build behind the room feature flag and apply no RLS until the bugfix council, the adversarial suite green on a shadow project, cross-platform verification, and **Rahul's written sign-off**.
> - Source: MEMBER-1 (`02` §3.4 / `03` §4.6a/§4.7a); PRD §5.7 REQ-LOCK-1/2; `CLAUDE.md` Tier-1; supersedes the prior preview-RPC-for-`accepted` framing.

> **⚠️ SEC-DECISION-1 — REMOVED (RULED by Rahul, 2026-07-18, SOR-1).** *[Margin note, 2026-07-18 — REVERSAL. This requirement previously specified an `is_admin()`-gated `set_application_decision(...)` SECURITY DEFINER RPC that WROTE `cohort_applications.status ∈ {accepted, waitlisted, rejected}` — "the one funnel status the app owns." SOR-1 **reverses** that: **TeleCRM is now the MASTER system of record and the app is a READ-ONLY MIRROR.** The sales team creates the funnel decision (including `accepted`) in TeleCRM; the app **never writes a funnel status** and there is **no in-app admin decision/acceptance-write RPC.** `set_application_decision` is deleted; no app-owned funnel-status write exists.]*
> **Acceptance is TeleCRM-sourced, and the app only READS it.** The `accepted` state reaches the app via the **read-only mirror/reconciler read path** (INTEG-KEY-1 phone-primary/email-fallback join; SEC-RECON-1 read-only externals) and/or a **TeleCRM webhook** if available — the reconciler/mirror is the authoritative accepted source. The acceptance experience is fired by the app **detecting the mirrored flip to `accepted`** (MEMBER-1 still READS `accepted` to gate the veil — §4, SEC-MEMBER-1 — it just never WRITES it). The dependency to spec: the mirrored `accepted` state must reach the app **promptly** (reconciler read cadence and/or the TeleCRM webhook) so the in-app acceptance experience fires without lag. INTEG-CRM-1 read-only holds: the app writes **nothing** back to TeleCRM.
> - Removed: the `set_application_decision` write RPC, its `is_admin()` authorization, its verdict-free-notification wiring, and the "reconciler never overwrites an app-written `accepted`" write-precedence rule (there is no app write to overwrite — the mirror is authoritative).
> - Source (superseded framing): `03` §2/§4.2; `04` §9; PRD §8.2 Open Q1; `02` §5 T6a — all carried the app-written-`accepted` carve-out that SOR-1 retires. See §6.3 (SEC-RECON-1) for the read-only mirror path that now sources `accepted`, and SEC-MEMBER-1 for the veil gate that reads it.
...
*[Margin note, 2026-07-18 — SOR-1: TeleCRM is the master system of record; the app is a read-only mirror.]* **This read path is now also the source of the `accepted` decision** (the deleted SEC-DECISION-1 previously had the app WRITE it). The reconciler/mirror reads TeleCRM's decision state by phone (primary) / email (fallback) — INTEG-KEY-1 — and **detecting the mirrored flip to `accepted` is what fires the in-app acceptance experience** (SEC-MEMBER-1 reads `accepted` to gate the veil). A TeleCRM webhook, if available, may push the flip in place of / alongside the poll so the app reacts promptly. The app writes **no** funnel status back (INTEG-CRM-1 read-only holds). Because TeleCRM has no literal `Accepted` status (it is implicit between `Interview completed` and `Converted`; `FUNNEL-DATA-AUDIT.md` §3, `02` §1 mismatch 1), deriving the `accepted` flip from the mirror is a reconciler-mapping concern to spec in `04`/`02`, not an app-side write.
...
flowchart LR
    subgraph READ["Read attacks — must return ZERO rows / RAISE"]
        ...
        A8[accepted_A SELECT any room-content table / no preview RPC] --> X8[0 rows / denied]
        A9[pre_member_A1 SELECT recordings/curriculum/assignments/feedback] --> X9[0 rows redacted]
    end
    subgraph WRITE["Write attacks — must REJECT"]
        ...
        W6b[pre_member_A1 INSERT post/reply into own room] --> Y6b[reject read-only]
    end
...
| **R10** | **MEMBER-1 `accepted` = zero room read:** `accepted_A` (confirmation-fee unpaid) reads every room-content surface for offering A — `cohort_announcements`/`cohort_room_posts`/`cohort_room_post_replies`/`cohort_resources`/`cohort_demo_entries`/`cohort_weeks`/`live_sessions`/`cohort_room_configs` — and there is **no `get_cohort_room_preview` RPC to call** | **0 rows / denied on every table; RAISE on any room RPC**; the confirm-screen veil renders from the `offerings` row + static chrome alone (assert no room-content read is issued) |
| **R11** | **MEMBER-1 `pre_member` redaction whitelist:** `pre_member_A1` (`confirmation_paid`) (a) reads the whitelisted subset (masthead/theme, this-week overview = session titles+dates, cohort-mate presence/count, announcements READ, upcoming-session calendar) and (b) attempts every redacted-out surface — full curriculum detail, **recordings**, assignment content/submission, **feedback**, mentor/instructor materials, and any community **write** (post/reply) | (a) **visible**; (b) **0 rows / denied** on every redacted surface and community write until `enrolled` — `pre_member` is read-only, its **own** access level (never widened into `member`) |
...
| **W10** | **No app-side funnel-status write (SOR-1, 2026-07-18 — replaces the removed SEC-DECISION-1 decision-write test):** assert there is **no** `set_application_decision` (or any) app RPC/path that writes `cohort_applications.status` to `accepted`/`waitlisted`/`rejected`; and `accepted` reaches the app **only** via the read-only TeleCRM mirror/reconciler | **no such write path exists** (the app never writes a funnel status); the mirrored `accepted` flip is the sole source and fires the veil gate (SEC-MEMBER-1) |

> **SEC-TEST-1 — The adversarial suite is a blocking gate, green on a shadow project, re-run every rooms phase.** `🔴 Tier 1`
> - Acceptance: `qa-harness/cohort-room-access.spec.mjs` runs as one command, exit 0, with every case above passing — including the **MEMBER-1 three-tier** cases (R10 `accepted` = zero room read + no preview RPC; R11 `pre_member` redaction whitelist; W6b `pre_member` community-write reject, on the `accepted_A`/`pre_member_A1` fixtures), the **cross-batch** cases (R8/R9/C3, on the two-batch offering-A fixture), the **lifecycle/revocation** cases (L1/L2, proving an enrolment flipped off `'active'` loses access and a re-grant restores it — the exact regression the resolver exists to prevent), and both canary strings (`LEAK_CANARY_A1`/`_A2`). Wired into `design-qa-gate` as the `room-access-leak` lens; the canary greps and the roster column-assert are part of the run. The suite is green on a shadow project **before** council and **before** any apply (SEC-APPLY-1). It re-runs on R1, R2, R3, R4.
> - **Prerequisite — the shadow must be ARMED first, every run: §7.1's four-step `shadow-grants.sql` recipe.** "Exit 0" only means something on a shadow that carries production's table grants; on an unarmed one every RLS assertion would pass for the wrong reason, which is why the suite aborts with **exit 2** instead of passing. Exit 2 is a setup step, not a finding (§7.1; `design-qa-gate.js:68-74`).
> - Source: `ROOMS-BACKLOG.md` R0-T4 (and R1/R2 "adversarial re-run" lines); PRD NFR-SEC-4; `ROOMS-ARCHITECTURE.md` §6.3.

### 7.1 The grant layer — the table-level SQL `GRANT`s that sit IN FRONT of RLS

**First, the word.** Everywhere else in this document a **grant** is a *membership* grant — "`accepted` holds **zero room read grant**" (§4), "**Manual grant**" (the §3.2 source table, row 4), `admin_grant_room_member` (`0002:198-211`). This subsection means something else, and something earlier: the **SQL `GRANT`** — the table-level privilege list (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, `REFERENCES`, `TRIGGER`) that the *database roles* `anon`, `authenticated` and `service_role` hold on a table. It is **not** one of the three access tiers (§4) and **not** membership. It is a **different and earlier gate**: Postgres checks the table ACL first and never consults a row policy for a verb the role does not hold.

| # | Lock | What it decides | Where it is stated |
|---|---|---|---|
| **1** | **Table ACL (SQL `GRANT`)** | Whether the role may attempt the verb *at all*. Denied here, RLS is never reached | The platform's create-time bootstrap + the `GRANT`/`REVOKE` blocks in `20260729100000` §7 and `20260729100100` §7; production's shape is captured in `qa-harness/shadow-grants.sql` |
| **2** | **RLS** | Which *rows* the attempted verb sees or touches | §5 of this doc; every policy routes through `cohort_room_can_access()` (SEC-RLS-1) |

Everything else in §7 proves lock 2. This subsection documents lock 1, because **a suite run against a database whose lock 1 is shaped differently from production's proves nothing about production** — and the **create-time default privileges** half of lock 1 is the part no migration in this repo reproduces. Be precise about the split: the room migrations' own 38 `GRANT`/`REVOKE` statements are lock 1 too, and they are what determines its end state (the table at the end of this subsection). What they cannot do is hand out what they then take back — that is the platform bootstrap's job, and it is the half a shadow has to be armed for.

**Production grants all seven verbs to every client role, and nothing in `supabase/migrations/` reproduces that.** The hosted platform runs a create-time bootstrap once per project — `ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO anon, authenticated, service_role` (`shadow-grants.sql` SECTION A, hand-maintained precisely because a per-table grant dump structurally cannot capture it) — so every table any later migration creates arrives with `anon`, `authenticated` and `service_role` already holding **`DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE`**. That exact seven-verb string is on **every** SECTION B row, generated from production (`ivkvluezuiojovpotlyb`) on 2026-07-28. **`ALL` is seven verbs, not four.** Each migration then `REVOKE`s down to what its table actually needs — which is why the room migrations' `REVOKE`s are mandatory rather than belt-and-braces (SEC-ENT-1), and why `qa-harness/shadow-grants.sql` exists at all.

**`TRUNCATE` bypasses RLS entirely — and is not client-reachable today. Say both halves.** `TRUNCATE` consults **no** row policy: not one of the 21 content policies is evaluated for it, including the ones protecting the rows the suite plants `LEAK_CANARY_A1`/`_A2` in. So a `REVOKE INSERT, UPDATE, DELETE` that omitted it left `authenticated` — the role every logged-in student carries — holding a verb that empties a cohort's noticeboard, library, feed, gallery and every recording position in one statement, with the membership wall never consulted. **Scoped honestly: PostgREST exposes no `TRUNCATE` verb, so there is no live client-reachable exploit today.** It is **defence in depth** ("the day someone adds a permissive route, that default is the difference between a bug and a leak") plus **claim accuracy** for anyone signing off on the grant surface — nothing to panic about and nothing to dismiss. Both migrations now revoke it, each in its own §7 grant block: `20260729100000_cohort_rooms_backbone.sql:1796-1797` (§7 `-- 7. RLS + grants.` opens at `:1725`), whose comment records at `:1782-1795` that this is exactly what contract note 10 was rewritten to say; and `20260729100100_cohort_room_content.sql:1148-1154` (§7 `-- 7. Grant hygiene.` opens at `:1104`), which carries the identical revoke for the seven content tables. `REFERENCES` and `TRIGGER` are deliberately **not** revoked: both require ownership of, or a matching privilege on, objects `authenticated` cannot reach, and neither reads nor destroys a row.

**A migrations-only database does not reproduce production's grants — so an unprepared shadow makes this whole suite green for the wrong reason.** Measured on a clean local stack, only **3 of 103** public tables grant `SELECT` to `anon`, and even `offerings` — which the public landing pages must read — grants nothing but `TRUNCATE`/`REFERENCES`/`TRIGGER` (`qa-harness/shadow-grants.sql:4-7`). After the file runs, **101** do: *derived*, not separately measured — SECTION B enumerates **101** tables that grant `anon` `SELECT` (the 102nd relation it names, `public_user_profiles`, carries no `anon` row). The consequence is the part that matters: **every RLS assertion against an ungranted shadow passes because permission is denied BEFORE RLS is consulted.** "`anon` cannot read this table" then proves only that `anon` was never granted the table — a green that means nothing, which is worse than no test at all. Every RLS suite is exposed to it, so every RLS suite has to be armed. On this branch that is exactly one: **`cohort-room-access`** — the only suite in `qa-harness/`, and the only one `package.json` declares (`test:room-access`). `shadow-grants.sql:14-15` names a second (identity-spine); that suite is task SP's, on a separate in-progress worktree, and is **not here to arm** — read that line as forward-looking, not as a second command this phase can run.

**The required ordering. `shadow-grants.sql` must be applied BEFORE any adversarial suite runs, and its own ORDERING block is the authority** (`shadow-grants.sql:19-30`) — reproduced here only so a reviewer knows it exists, not to replace it.

**⚠️ THE TARGET IS A DISPOSABLE SHADOW PROJECT, NEVER PRODUCTION (`ivkvluezuiojovpotlyb`). READ THIS BEFORE COPYING A LINE OUT OF THE TABLE BELOW.** Step 0 drops schema `public` and wipes the migration ledger; step 1 writes a `pg_default_acl` entry that every future migration on that database inherits; step 2 writes schema. None of it is undone by re-running anything, and this recipe is normally handed to an agent **together with a connection string the agent did not choose** (`design-qa-gate.js` supplies it). So the target is checked twice before a single statement runs, and both checks are hand-maintained at the source and marked non-droppable on regeneration (`shadow-grants.sql:107`):
- **`-v ROOM_QA_SHADOW=1` is not a flag, it is a refusal interlock.** The file aborts before its first statement when the marker is absent — "This file permanently alters a database's grant model … meant to be run only against a disposable SHADOW project, never against production (ivkvluezuiojovpotlyb) … If you were about to run this against production: do not" (`:107-148`). It is an affirmation, not a value: its *presence* is what is tested, so a half-remembered one-liner or a pasted command that lost its target cannot execute. It is **mandatory on both psql passes** (`design-qa-gate.js:73`).
- **A production deny-list backs it up.** Any connection whose role, database or `application_name` mentions the production ref is refused outright, marker or no marker — Supabase's pooled connections carry the ref in the role name (`postgres.<ref>`), so a prod pooler string identifies itself (`:150-170`). `ON_ERROR_STOP` makes both terminal rather than advisory.

Nothing in this recipe is needed on production: production already has these grants, which is where they were copied from. The QA gate carries the same two rules verbatim (`design-qa-gate.js:73-74`, "never prod `ivkvluezuiojovpotlyb`").

| Step | Do | Why this order |
|---|---|---|
| **0** | **Empty the shadow** — and only ever a shadow (`DROP SCHEMA public CASCADE; CREATE SCHEMA public;` + wipe `supabase_migrations.schema_migrations`) — or create a fresh project. **`supabase db reset` does NOT satisfy this** — it re-applies the migrations and re-creates the failure | Pass 1 must run against a database where the nine R0 tables do not exist yet |
| **1** | `psql … -v ROOM_QA_SHADOW=1 -f qa-harness/shadow-grants.sql` — **before** `db push`; the marker is the refusal interlock above, not decoration | **This is the pass people miss.** SECTION A arms `ALTER DEFAULT PRIVILEGES`, which Postgres consults at `CREATE TABLE` time and never again — it **cannot be retrofitted** onto tables that already exist. The same pass restores the `USAGE ON SCHEMA public` that step 0's drop took away |
| **2** | `supabase db push --db-url "$SHADOW_DB_URL"` | Builds the schema, so the nine R0 tables are created *under* the armed bootstrap |
| **3** | Run `shadow-grants.sql` again — **after** `db push`, same line, marker included | SECTION B grants on tables *named* from prod; on pass 1 they do not exist yet and are skipped by design |
| **4** | …then, and only then, the adversarial suites | Run earlier, `cohort-room-access.spec.mjs` **refuses to run and exits 2** — it does not hand you the vacuous green above, because its PRECONDITION exists to catch exactly that state |

**What a skipped step actually looks like: exit 2, not a false green.** The vacuous green is the failure the suite was built to make *unreachable* — it is what you would get without the guard, not what you will see. `cohort-room-access.spec.mjs` hard-aborts (`die(msg, code = 2)`, `:483`) on four distinct PRECONDITION signatures, each naming the step that was skipped: no `USAGE` on schema `public` (`:1240-1252` — "the signature of a database emptied for step 0 and then pushed WITHOUT pass 1"), the R0 migrations absent (`:1254-1264`), the shadow ungranted (`:1265-1282` — "REFUSING TO RUN, BECAUSE EVERY RLS ASSERTION … WOULD PASS VACUOUSLY AND THE RUN WOULD PRINT A GREEN SUMMARY THAT PROVES NOTHING"), and built in the wrong order, keyed on the absent bootstrap-only `REFERENCES`/`TRIGGER` pair (`:1283-1300`). Every abort reprints this four-step recipe verbatim under a **`FIX —`** heading (`:1135`); if what you ran differs from the printed one, run that first. **Exit codes: 0 = the wall holds · 1 = a leak (critical) · 2 = the suite could not run · 3 = `--list` was passed.** A non-zero exit is never a pass — and a **2 is a SETUP STEP, not a finding**: report it only if it persists after all four steps have actually been carried out as written (`design-qa-gate.js:68`, `:73`). The suite says the same in its own inventory line: the PRECONDITION "aborts (exit 2) if it does not" (`:490`).

The suite carries its own arming controls for exactly this reason: `GRANT.0a` reads the `pg_default_acl` entry SECTION A writes, `GRANT.0b` proves it *fires* (a table created through the same connection `db push` uses arrives holding all seven verbs), and `GRANT.1`/`GRANT.2` report **VACUOUS as a failure** rather than a pass when the bootstrap is not armed. Schema `USAGE` is checked separately by the spec's PRECONDITION, because `has_table_privilege()` cannot see it.

**The measured end state for the nine R0 tables.** Recorded verbatim in the backbone's A6 item (9), "GRANT END-STATE UNDER AN ARMED BOOTSTRAP" (`20260729100000_cohort_rooms_backbone.sql:2630-2660`): the bootstrap armed first, the nine tables created, then all 38 `GRANT`/`REVOKE … ON public.<one of the nine>` statements in `20260729100000` + `20260729100100` replayed in file order (`20260729100200` issues none, so the set is complete). For `authenticated` — **`anon` holds ZERO rows**:
...
| `cohort_room_posts` | `DELETE, REFERENCES, SELECT, TRIGGER, UPDATE` — **no `INSERT`** |
| `cohort_room_post_replies` | `DELETE, REFERENCES, SELECT, TRIGGER, UPDATE` — **no `INSERT`** |

Read three things off it. **`TRUNCATE` appears on no row.** The two membership/config tables hold **`REFERENCES, SELECT, TRIGGER` only** — no DML. The two feed tables hold `DELETE` and `UPDATE` but **not `INSERT`**, because the write RPC is the sole write path (SEC-WRITE-1).

**The old shorthand "`SELECT` and nothing else" is FALSE, and must not be reinstated.** `REFERENCES` and `TRIGGER` genuinely remain, by design. The backbone's A6 item (6), "WITHDRAWN CLAIM, KEPT BECAUSE THE REASON IS THE POINT" (`:2545-2578`), withdrew that wording and **forbids restating it**, for a reason this document has to carry: on a harness that never runs the platform bootstrap, observing "SELECT and nothing else" measures **the ABSENCE OF THE BOOTSTRAP, not the presence of the revoke** — the exact false-assurance failure above — and on production, where the bootstrap *has* run, the claim was untrue when it was written, because `authenticated` retained `TRUNCATE`. Do not contradict the migration: the per-verb expectation is the table above, where `REFERENCES` and `TRIGGER` are expected and deliberate and **`TRUNCATE` on any row is the failure**.

**And this is not a shadow and not prod.** A6 item (9) carries its own caveat: it proves that *those* statements over *that* default-privilege state produce that end state. It assumes prod's default privileges are the `GRANT ALL` shape `shadow-grants.sql` captured, and it cannot see a manual grant made on the target outside these migrations. **the THIRD CHECK in the backbone's own section 8 still has to run on the target after the push** (`-- THE THIRD CHECK — THE GRANTS` at `20260729100000_cohort_rooms_backbone.sql:2335`, inside the section-8 POST-PUSH VERIFY block that opens at `:2263` — the migration's section 8, not §8 of this doc) and paste its rows, not a verdict. Item (9) narrows what that check is looking for; it does not replace it. **"The target" here means whichever database the push landed on — the provisioned shadow under review, or production at R0-T5. It is a read-only verification target and never a step-0 drop candidate:** the provisioning recipe above (schema drop, ledger wipe, `pg_default_acl` write) belongs to a disposable shadow only, never to `ivkvluezuiojovpotlyb`.
...
| SEC-STATE-1 | `accepted` (unpaid) fixture → 0 rows on content **and** `cohort_weeks`/`cohort_room_configs`; veil renders from the `offerings` row + static chrome only (no config/week read, **no preview RPC**); lobby is net-new, not a flag flip |
| SEC-MEMBER-1 | Three-tier boundary: `accepted` → 0 rows on every room-content table + **no `get_cohort_room_preview`** (marketing-class veil only, zero room read grant); `pre_member` (`confirmation_paid`) reads only the whitelisted pre_start subset and is denied on recordings/curriculum/assignments/feedback/mentor-materials/community-write until `enrolled`; `enrolled` → full `member`; `member` never widened to include `pre_member` |
...
| SEC-TEST-1 | `cohort-room-access.spec.mjs` exit 0 on a shadow **armed by §7.1's four-step `shadow-grants.sql` recipe first** (unarmed → exit 2, a setup step, never a pass), before council/apply; two-batch fixture proves cross-batch isolation (R8/R9/C3); lifecycle/revocation proven (L1/L2); both canary greps + column-assert; re-runs every phase |
...
1. **LOBBY-1 / R-D2 — RESOLVED (Rahul, 2026-07-18, MEMBER-1).** The pre-start lobby is **ON**, but the entry gate is **`confirmation_paid`** (a scoped `pre_member` row), **not `accepted`** — `accepted` never enters a lobby (zero room read grant). Still **net-new work** (new resolver branch on the `confirmation_payment_id` stamp + new `cohort_applications` trigger + `cohort_room_in_lobby()` helper), not an existing flag. See SEC-MEMBER-1 for the redaction whitelist (line items remain Rahul-tunable). (§4/§5.4; PRD R-D2)
2. **VEIL-SOURCE-1 — RESOLVED (Rahul, 2026-07-18, MEMBER-1).** The `accepted` veil is built **only** from public-safe / marketing-class data (the `offerings` row + static module chips), never real room rows; the redacted-preview RPC path (`get_cohort_room_preview`) is **deleted** and no new non-member-readable projection is built. No new read surface, no RLS change. See SEC-MEMBER-1 / SEC-STATE-1. (§4/§5.4; PRD §5.7)
...
4. **System of record — RESOLVED (Rahul, 2026-07-18, SOR-1).** *[Margin note, 2026-07-18 — REVERSAL: the prior v1 default here was "reconciler + app owns what it controls," AND doc 05 previously carved out `accepted` as the one app-WRITTEN funnel status (the now-removed SEC-DECISION-1). SOR-1 supersedes both.]* **TeleCRM is the MASTER system of record; the app is a READ-ONLY MIRROR.** The app **never writes a funnel status** — no admin-gated state-transition RPC, no acceptance-write path (SEC-DECISION-1 is removed). The sales team creates funnel events (including `accepted`) in TeleCRM; the app READS/mirrors them and reacts (detecting the mirrored flip to `accepted` fires the acceptance experience; §6.3, SEC-RECON-1). The app still writes only what it *owns operationally* — provisioning, payment-driven states it captures via its own Razorpay path, and room membership — none of which is a TeleCRM funnel status. INTEG-CRM-1 read-only holds. (`FUNNEL-DATA-AUDIT.md` §5/§6; `02` §1 SOR-1)
