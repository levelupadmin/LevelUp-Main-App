...
**P5 — The Admin (Rahul / ops).** Configures offerings, batches, weeks, and room theming; enrolls and assigns batches. _(Per ROUND-F SOR-1, 2026-07-18: interview outcomes and acceptances are recorded by the sales team in **TeleCRM**, the master system of record — the admin no longer writes them in-app; the app mirrors them.)_ Wants: a new cohort with its own look and vocabulary to cost *one config row and zero deploys* (`ROOMS-ARCHITECTURE.md` §1). Fails today by: intermediate funnel states having no writer, and per-cohort identity requiring code (ibid; `FUNNEL-DATA-AUDIT.md` §2).
...
- **CRO structural bets — RULED (ROUND-F, 2026-07-18):** #2 = IN (absorbed into v2 Stage 05); **#1 (funnel inversion) and #3 (honors tiers) = OUT of v1, parked fast-follow.** Each is carried below under its RAHUL DECISION banner with the ruling applied. _Margin note (2026-07-18 · reverses the prior "#1/#2/#3 all IN as recommended scope" lean): #1 is retired for v1 by **INTEG-PAY-1** (do-not-touch the existing Tally→Razorpay intake chain) and #3 by **STANDING-1** (no honors tiers in v1; single Completion certificate)._
...
|---|---|---|---|
| **Slice 1 — Make the funnel observable & recoverable** | REQ-RECON-1 (reconciliation read path) · Stage 01 identity spine (REQ-IDENT-1..4) · Stage 03/04 reminder ladder + open loop (REQ-INSTALL-*, REQ-LOOP-*) · staged-applicant home + the **abandoned-application re-entry nudge** | Turns the NSM from unmeasurable to measurable and recovers the ~69% contactable abandoners **via a post-intake experience + a re-entry nudge that reads partial state (TeleCRM/webhook) but NEVER touches the Tally/Razorpay intake chain** (ROUND-F INTEG-PAY-1) — **without one line of room schema, and without modifying the intake forms.** Auth + reconciliation are the only Tier-1 items. | Auth (contained) — the intake chain stays untouched (INTEG-PAY-1) |
...
> _Margin note (2026-07-18, ROUND-F INTEG-PAY-1 · reverses the prior Slice-1 form-restructure lever):_ **REQ-APP-3 (form-shortening) and CRO-1 (funnel inversion) are removed from Slice 1 / v1** — Rahul ruled the existing Tally→in-form-Razorpay-₹400→Calendly→app intake chain **do-not-touch**, and both levers require Tally-builder changes he has deferred. **The north star (§2.1) still holds unchanged; only the v1 conversion lever shifts — from form-restructure to a POST-intake experience + the abandoned-application re-entry nudge** (reads partial state, nudges without touching the form). The re-entry nudge and REQ-RECON-1's contactable-partial marker are now Slice 1's whole conversion contribution. Form-restructure/inversion return as a fast-follow A/B (§4.3) once Tally changes are approved.
...
Plus the three structural CRO bets, each RULED below (ROUND-F, 2026-07-18) — **only CRO-2 survives into v1; CRO-1 and CRO-3 are ruled OUT of v1 and parked fast-follow:**

> **RAHUL DECISION — CRO-1: Invert the funnel — pay first, qualify after. → RULED OUT of v1 (ROUND-F INTEG-PAY-1, 2026-07-18); parked as a fast-follow A/B.**
> _Margin note (2026-07-18 · reverses the prior "IN as a Tally-side A/B, v1-prepared" recommendation **and** the straight-ship REQ-APP-3 commitment in the note below):_ Rahul ruled **INTEG-PAY-1 — DO NOT modify the existing intake chain.** The Tally application forms stay EXACTLY as they are: clicking "Apply" opens the existing Tally form → on submit it goes to the existing in-form Razorpay ₹400 link → then Calendly → then the app. The app inserts NOTHING into this chain and does NOT route ₹400 through its own order path.
> **Consequences for v1:** (1) **CRO-1's funnel inversion is OUT of v1** — it reorders the intake and would touch the form + payment gate the ruling freezes. (2) **REQ-APP-3's straight-ship form-shortening is ALSO OUT of v1** — every one of its recs (progress bar, split page 5, optional Q7/Q9, cut the quiz block, shorten to ~12–14 fields, fix stale dates) is a Tally-builder change to a do-not-touch form. Both are **parked as a fast-follow A/B** (§4.3 #15); the idea is NOT deleted — it is gated on the Tally changes Rahul has deferred.
> **What replaces the form lever in v1:** the app's conversion role is now purely a **POST-intake experience + the abandoned-application RE-ENTRY nudge** — it reads partial state via TeleCRM/webhook and nudges the applicant *without touching the form* (see the Slice-1 note in §4.0 and REQ-RECON-1's contactable-partial marker). The **north star (§2.1) is unchanged**; only the lever shifts (form-restructure → post-intake + re-entry).
> **When it later ships (fast-follow):** the cost is still **not** "near-zero app-engineering" — moving qualification post-payment needs either a **second Tally form** (new webhook mapping + changed provisioning timing) or an **in-app post-payment intake**; and the historical evidence remains that form length is the dominant lever (`TALLY-UX-ANALYSIS.md` §1: 91%→26%→14% by length; Walls 2/3 at Q13/Q17 bracket ~44% of abandonment, and the 4 quiz questions "collect no qualification signal at all," ibid §4/§5). `🟡 Tier 2` when built (Tally + Razorpay config *plus* a second-form/intake mapping — not a no-op).
...
> **RAHUL DECISION — CRO-3: Certificates with honors tiers. → RULED OUT of v1 (ROUND-F STANDING-1, 2026-07-18). v1 ships a single "Completion" certificate; tiered standing is parked fast-follow.**
> _Margin note (2026-07-18 · reverses the prior "IN as a Stage-08/12 computed standing" recommendation):_ Rahul ruled **STANDING-1 — NO honors tiers in v1; a single "Completion" certificate.** Remove all Distinction / Merit / provisional-cutoff language and any "live academic standing from week 1" surface for v1. **The certificate is issued on cohort completion.** Attendance is still **TRACKED and SHOWN in-room** (the record + recovery-via-recordings stays, so recordings remain *recovery*, not a substitute for attending) but it does **NOT** define certificate tiers. Whether attendance gates the completion cert at all is a minor Rahul decision — **default: issued on completion, no hard attendance gate in v1.** The tiered "academic standing" idea is NOT deleted — it returns as a fast-follow once Rahul sets cutoffs. `🟡 Tier 2` when it later ships (computed view over existing attendance/submission tables). See STANDING-1 (§5.8) and REQ-ROOM-5 / REQ-FINISH-1, now superseded on the tiers point.

#### ENTRY-PARITY — the app-vs-marketing-landing equivalence requirement (new, ROUND-F, 2026-07-18)

> **REQ (scope-level) — ENTRY-PARITY-1:** Whether a user applies **via the app** or **via a marketing landing page**, the experience and the data wiring must be **EQUIVALENT** — both paths yield the same flow (existing Tally form → in-form Razorpay ₹400 → Calendly → app) and the same reconciled first-party data (REQ-RECON-1 joins on phone-primary/email-fallback, per INTEG-KEY-1). Neither path forks the intake chain (INTEG-PAY-1) or produces a user the reconciler can't resolve. **Calendly MAY optionally be embedded in-app** for UI control (nice-to-have, not required for v1); embedded or linked-out, the flow and data are identical. _Source: ROUND-F INTEG-PAY-1 (entry-parity clause). A fuller functional statement belongs alongside Stage 01/05 in §5; this scope-level requirement fixes the contract._
...
| Prio | CRO # | Item | Note |
|---|---|---|---|
| 1 | #15 | Name & run the A/B tests — **now including form-shortening (REQ-APP-3) and fee inversion (CRO-1)**, success-page slot embed, essay before/after | The **A/B assignment/measurement harness** is fast-follow. Per ROUND-F **INTEG-PAY-1** (do-not-touch intake chain), both CRO-1 *and* REQ-APP-3's form-shortening are now parked into this bucket — all Tally-side levers are fast-follow-*validated*, gated on the Tally changes Rahul deferred. v1's own conversion lever is the post-intake re-entry nudge (Slice 1, §4.0), which needs no harness. Needs the §7 events. |
...
| ~~8~~ | ~~#5~~ | ~~"₹400 credited toward tuition if accepted" (reframe fee → deposit)~~ **RETIRED (ROUND-F FEE-1, 2026-07-18)** | **Ruled out.** The ₹400 is a **separate NON-REFUNDABLE review fee, NOT credited toward tuition** — see RAHUL DECISION FEE-1. Remove the deposit-reframe / tuition-credit idea from the backlog. |
...
> **RAHUL DECISION — FEE-1: Credit the ₹400 toward tuition if accepted? → RULED (ROUND-F, 2026-07-18): NO. The ₹400 is a separate NON-REFUNDABLE review fee, NOT credited toward tuition.**
> _Margin note (2026-07-18 · reverses the prior "yes, credit it" fast-follow lean):_ `CRO-SUGGESTIONS.md` #5 argued to credit it; Rahul ruled the opposite. The ₹400 is a **separate, non-refundable review fee** — **remove all "credited toward tuition" / deposit-reframe language and any tuition-credit ledger logic** wherever it appears (fast-follow #5 is retired above; the payment-ledger fast-follow #14 and the enrollment-details screen must NOT show a ₹400 tuition credit). The fee stands on its own as the cost of a review slot.
...
*Note for the crew:* the intermediate transitions (`interview_scheduled`, `interview_done`, `accepted`, `rejected`) **have no writer in the codebase today** (`FUNNEL-DATA-AUDIT.md` §2). **Under ROUND-F SOR-1 (2026-07-18) the app never *writes* these — it MIRRORS them.** _Margin note (2026-07-18 · reverses the earlier "the app becomes the writer / `accepted` is app-written" framing):_ TeleCRM is the master system of record; the app **reconciles** these states from TeleCRM/Calendly/Razorpay by phone (primary) / email (fallback) — including `accepted`, whose flip in TeleCRM is what fires the in-app acceptance experience (via REQ-RECON-1's read path and/or a TeleCRM webhook). Reconciliation-not-authorship is the central engineering theme of Stages 01, 05, and 06 (see §8.2 Q1).
...
> _Margin note (2026-07-18 · reverses "shipped straight in v1"):_ Rahul ruled **INTEG-PAY-1 — do-not-touch the existing Tally intake chain.** Every rec below is a Tally-builder change to that frozen form, so **REQ-APP-3 is out of v1 and parked as a fast-follow A/B** (§4.3 #15), gated on the Tally changes Rahul deferred. The evidence and the recs stay recorded here for that fast-follow; nothing below ships in v1. v1's conversion lever is the post-intake re-entry nudge (§4.0), not the form.
- Why it was proposed (retained as fast-follow rationale, no longer a v1 commitment): form length is "the dominant lever on completion" (`TALLY-UX-ANALYSIS.md` §1), and Walls 2 (Q13, 459 stalls) + 3 (Q17, 423 stalls) — the four quiz questions that "collect no qualification signal at all" — bracket ~44% of all abandonment (ibid §4/§5). The fixes are near-zero-engineering Tally-builder changes that keep field *order* and the payment gate intact, so they carry none of CRO-1's inversion risk and do **not** wait on the A/B harness (§4.3 #15). This is the resolution of the circular-scope trap in §4.1.
- Behavior (parked fast-follow — audit recs 1–7): <!-- 2026-07-18: was "Behavior, committed for v1 (audit recs 1–7)"; reframed per ROUND-F INTEG-PAY-1 — these are parked fast-follow Tally changes, NOT a v1 deliverable. -->
  <!-- Each rec below is a Tally-builder change to the frozen intake chain; none ships in v1 (see margin note above). -->
...
- Fast-follow acceptance (when this ships as the §4.3 #15 A/B, not v1): <!-- 2026-07-18: was a v1 "Acceptance:" block; recast as a fast-follow criterion per ROUND-F INTEG-PAY-1 — nothing here is a v1 exit gate. --> The reworked form renders a progress indicator; the quiz block is ≤2 required questions; contact (name/WhatsApp/email) sits on its own page before any quiz wall; Q7 and Q9 are optional; availability options contain only forward-dated cohort starts (no past dates); the essay label reads as a light 2–3-sentence ask; answerable-field count ≤14. (All verifiable via the Tally form structure read the audit already used.) The ₹400 gate position and field order that CRO-1 would change are **not** touched here — those remain the CRO-1 A/B. Because these depend on the Tally changes Rahul deferred, none of this is a v1 acceptance gate; v1 acceptance for Stage 02 is carried by REQ-APP-1/REQ-APP-2 only.
...
- Behavior: Every session's `recording_url` renders as a shelf row with a "Continue watching" rail on top (resume position via new `cohort_recording_progress`, 5–95%). This closes G2 — the reminder email already promises "the recording will be on your cohort dashboard within 24 hours" and the product currently never renders it. Recordings are reframed as *recovery* (a make-up path that keeps attendance whole), not a substitute for attending. _(ROUND-F STANDING-1: no honors tiers in v1, so recovery no longer "protects a standing tier" — it simply restores attendance credit.)_
...
**REQ-ROOM-5 — The record with a recovery path.** `🟡 Tier 2`
> _Margin note (2026-07-18 · reverses the "+ honors standing" / "academic standing (Distinction / Merit / Completion) from week 1" clauses per ROUND-F STANDING-1):_ **No honors tiers in v1.** v1 shows the attendance record + recovery path only; **drop the live standing line and the Distinction/Merit tiers.** Attendance is still tracked and shown in-room (recovery via recordings stays), it just does not compute a tier. The tiered standing line is fast-follow.
- Behavior: The student sees their own ledger — attendance (per week, present/recovered/missed), assignments, recoveries used, and certificate-eligibility (a single "Completion" cert, per STANDING-1). A missed week has a recovery path (watch the recording + post a 3-line recap within 6 days → marked "Recovered," distinct from present, and it counts). Registrar language only — no streaks, no flames, no points. **No honors-tier "academic standing" line in v1 (ROUND-F STANDING-1); attendance is tracked and shown but computes no tier.**
- Acceptance: The record renders real attendance data; recovery marks a week "Recovered" distinct from "present"; no gamification affordances (grep for streak/points/XP = 0); **no Distinction/Merit standing line renders (grep = 0).**
- Implements: Screen 8E. Source: `COHORT-LOGIC.md` §2 (attendance/threshold); `CRO-SUGGESTIONS.md` #3; §6 Rule 04.

> **RAHUL DECISION — STANDING-1: the honors-tier cutoffs. → RULED (ROUND-F, 2026-07-18): NO honors tiers in v1; a single "Completion" certificate.**
> _Margin note (2026-07-18 · reverses the prior "provisional Distinction ≥90% / Merit ≥85% cutoffs" default):_ Rahul ruled **no honors tiers in v1.** **Remove Distinction / Merit / provisional-cutoff language and any "academic standing from week 1" surface.** The certificate is **issued on cohort completion.** Attendance is still **TRACKED and SHOWN in-room** (the record + recovery-via-recordings stays) but does **NOT** define certificate tiers. Whether attendance gates the completion cert at all is a minor Rahul decision — **default: issued on completion, no hard attendance gate in v1.** The tiered standing idea is not deleted — it returns as fast-follow once Rahul chooses to set cutoffs; until then there are no tiers to build. _(Prior recorded default, now superseded, for the fast-follow: Distinction ≥90% attendance + all assignments; Merit ≥85% + ≥80% assignments; Completion = the 85% eligibility gate.)_
...
> _Margin note (2026-07-18 · reverses "standing tier per CRO-3" per ROUND-F STANDING-1):_ **No honors tiers in v1** — the transcript carries attendance %, recovered weeks, assignments, and mentor grade, but **no standing tier**; the certificate is a single **"Completion"** certificate issued on cohort completion (default: no hard attendance gate in v1). Tiered standing is fast-follow.
- Behavior: The record becomes a transcript (attendance %, recovered weeks, assignments, mentor grade — **no standing tier in v1**, STANDING-1); the certificate is a single "Completion" certificate issued on completion; it gains a public verify URL backed by the transcript (revocable by the student) and share art at acceptance-card quality.
...
> **ROUND-F — RULED by Rahul 2026-07-18 (these nine are decided, not defaults).** SOR-1, INTEG-KEY-1, INTEG-PAY-1, INTEG-CRM-1, INTEG-CAL-1, ROSTER-SCOPE-1, STANDING-1, FEE-1, PILOT-1 are now **RULED** and override the prior recommended defaults where noted. The "Recommended default" column below carries the RULED outcome for these rows. Reversals applied: **accepted is TeleCRM-sourced, not app-written** (SOR-1); **no honors tiers, single Completion cert** (STANDING-1); **₹400 non-refundable, not credited** (FEE-1); **form-inversion/shortening parked** (INTEG-PAY-1); **Creator Academy pilots first, then AI + BFP/VE together** (PILOT-1).

| ID | Decision | Recommended default (RULED where marked) |
|---|---|---|
| **SOR-1** `🔴` **RULED** | System of record for funnel status (incl. acceptance) | **TeleCRM is the MASTER system of record; the app is a READ-ONLY MIRROR.** The sales team creates funnel events (including `accepted`) in TeleCRM; the app READS/mirrors them and reacts (detecting the flip to `accepted` fires the in-app acceptance experience). The app **NEVER writes a funnel status.** ⚠️ Reverses the earlier "accepted is the one app-WRITTEN status" resolution and **removes the in-app admin decision RPC (SEC-DECISION-1).** Acceptance reaches the app via REQ-RECON-1's read path and/or a TeleCRM webhook. **MEMBER-1 stays** — the app still READS `accepted` to gate the veil, it just doesn't write it (§8.2 Q1, §5 state note) |
| **INTEG-KEY-1** **RULED** | Cross-system join key | **Phone is PRIMARY, email is FALLBACK** (confirms the default). The reconciler tries phone first, email second, and records which resolved (REQ-RECON-1, §5.1) |
| **INTEG-PAY-1** **RULED** | The intake/payment chain | **DO NOT modify the existing intake chain.** "Apply" opens the existing Tally form → existing in-form Razorpay ₹400 link → Calendly → app. The app inserts nothing and does not route ₹400 through its own order path; the reconciler NET-matches legacy Razorpay payments back to app users by phone. ⚠️ **Retires for v1: CRO-1 (funnel inversion) + REQ-APP-3 (form-shortening)** — parked fast-follow (needs Tally changes Rahul deferred). Adds **ENTRY-PARITY** (§4.1 ENTRY-PARITY-1). The app's v1 conversion role = post-intake experience + abandoned-application re-entry nudge (§4.0/§4.1) |
| **INTEG-CRM-1** **RULED** | TeleCRM write-back in v1 | **Read-only in v1** (no write-back). Confirmed |
| **INTEG-CAL-1** **RULED** | Calendly account plumbing | **Org-level subscription; v1 plugs in ONE existing Calendly account.** Fast-follow: switch between LevelUp's TWO Calendly accounts by availability (deferred; v1 single account) (§5.5) |
| **ROSTER-SCOPE-1** **RULED** | Roster scope | **Batch-scoped** — a student sees only their own batch's cohort-mates, not the whole offering (confirms default) (§5.7, REQ-LOCK-3) |
| **PILOT-1** **RULED** | Pilot sequence / rollout ladder | **Creator Academy FIRST (already live), then AI cohort + BFP/VE launched TOGETHER as the second wave.** ⚠️ Changes the earlier "new Live-Filmmaking batch pilots first" default. Creator Academy is live, so the pilot runs alongside its existing batch **without disruption** (extra care vs a fresh batch) |
| **BUILD-1** | The v1 build order | Three slices, funnel-first, rooms-last: (1) reconciliation + spine + ladder + **abandoned-application re-entry nudge** (form-shortening removed per INTEG-PAY-1), (2) interview + decision (PNG/WebM), (3) rooms R0–R4 (§4.0) |
...
| **CRO-1** **RULED OUT** | Invert the funnel (pay first, qualify after) | **OUT of v1, parked fast-follow** (ROUND-F INTEG-PAY-1) — reverses the prior "IN as an A/B" lean; the inversion touches the do-not-touch intake chain and needs Tally changes Rahul deferred (§4.1/§5.2) |
| **CRO-2** | Interview slots on the ₹400 success screen | IN — absorbed into v2 Stage 05; hold as the second A/B (§4.1) |
| **CRO-3** **RULED OUT** | Certificates with honors tiers | **OUT of v1, parked fast-follow** (ROUND-F STANDING-1) — reverses the prior "IN as a computed standing" lean; v1 ships a single "Completion" certificate, no tiers (§4.1/§5.8) |
| **STANDING-1** **RULED** | Honors-tier cutoffs (Distinction/Merit/Completion) | **NO honors tiers in v1; single "Completion" certificate** — reverses the prior provisional-cutoff default. Remove Distinction/Merit/cutoff language. Attendance still tracked + shown in-room (recovery via recordings stays) but does NOT define tiers. Cert issued on completion; **default no hard attendance gate in v1** (minor Rahul decision). Tiered standing = fast-follow (§5.8) |
| **FEE-1** **RULED** | Credit the ₹400 toward tuition if accepted | **NO — the ₹400 is a separate NON-REFUNDABLE review fee, NOT credited toward tuition** — reverses the prior "yes, credit it" lean. Remove all credit/deposit-reframe language and tuition-credit ledger logic (§4.3/§8) |
...
1. ~~**Which is the system of record going forward**~~ **— RESOLVED (ROUND-F SOR-1, 2026-07-18).** _Margin note (2026-07-18 · reverses the earlier "`accepted` is the one app-WRITTEN status via an `is_admin()`-gated Stage-06 admin decision" carve-out):_ **TeleCRM is the MASTER system of record; the app is a READ-ONLY MIRROR.** The sales team creates every funnel event — *including* `accepted` — in TeleCRM; the app **READS/mirrors** them (phone-primary/email-fallback per INTEG-KEY-1) and **reacts** (detecting the flip to `accepted` fires the in-app acceptance experience). The app **NEVER writes a funnel status**, and **the in-app admin decision RPC (SEC-DECISION-1) is removed.** Dependency to spec: the acceptance experience needs the TeleCRM `accepted` state to reach the app **promptly** — via REQ-RECON-1's reconciler read path and/or a **TeleCRM webhook** if available. **MEMBER-1 is unaffected** — the app still READS `accepted` to gate the veil (it just no longer writes it). TeleCRM stays read-only for writes (INTEG-CRM-1). (`FUNNEL-DATA-AUDIT.md` §5/§6; canonical statements: `02-STATE-MACHINE.md` §1, `03-DATA-MODEL-ERD.md` §2, `04-INTEGRATION-CONTRACTS.md` §9 — updated in the same ROUND-F pass)
...
6. ~~**Honors-tier thresholds (CRO-3)**~~ **— RESOLVED (ROUND-F STANDING-1, 2026-07-18): no honors tiers in v1.** v1 ships a single "Completion" certificate; there are no cutoffs to set. The only remaining minor Rahul decision is whether attendance gates the completion cert at all — **default: issued on completion, no hard attendance gate in v1** (§5.8). Tiered standing returns as fast-follow if/when Rahul wants it.
