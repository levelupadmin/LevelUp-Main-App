...
| `accepted` | `accepted` | *(no TeleCRM equivalent — acceptance is unreadable from TeleCRM; §1 mismatch 1)* | Sealed decision 6A → reveal → card 6C → claim 6E (**confirm CTA present**) | **E2** Veil-only (no membership) | `[written]` — the one carve-out: app admin decision at Stage 06 (SOR-1), not reconciled |
