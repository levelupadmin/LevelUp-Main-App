...
- **The truth this doc is built around (now the RULED design, not just a gap):** the app is **not** the system of record — TeleCRM is (**SOR-1, RULED 2026-07-18**; §1). Across 199 recent Razorpay payments, **0** carried an application/offering/order id (`FUNNEL-DATA-AUDIT.md §2`), and the intermediate states (`interview_scheduled`, `interview_done`, `accepted`, `rejected`) **have no writer anywhere in the codebase** (ibid) — and under SOR-1 they intentionally never will: the app **mirrors** them from TeleCRM. So these states are **reconciled** (derived by reading Tally/TeleCRM/Razorpay by phone+email, per **REQ-RECON-1**, `01-PRD.md §5.1`), not **written**. Each state is tagged with which it is.
...
|---|---|---|---|---|
| **A** | **App** — `cohort_applications.status` | The clean, intended funnel status enum | `submitted · app_fee_paid · interview_scheduled · interview_done · accepted · rejected · confirmation_paid · balance_paid · enrolled · withdrawn · waitlisted` (`migration 20260413100000:16-17`) | **Mirror, not writer (SOR-1, 2026-07-18).** The app writes **no funnel status** going forward — it mirrors TeleCRM. It writes only identity (`auth.users`) and room **membership** (MEMBER-1). Money advances mirror the existing Razorpay chain (₹400 is app-external, INTEG-PAY-1; only the in-app ₹8k/balance staged verify writes `confirmation_paid`/`balance_paid`). The interview/decision states have **no app writer** (`FUNNEL-DATA-AUDIT.md §2`) — that is now the intended v1 model, not a gap. |
| **B** | **TeleCRM** — top-level `status` picklist | The *operational* funnel — where the money actually flows | `NEW · DNP 1 · DNP Reminder · Direct Junk · WARM · HOT · Fee Link Sent · Application Fee Paid · Interview Scheduled · Need to reschedule interview · Interview completed · No show · Deffered · Converted · Lost` (`FUNNEL-DATA-AUDIT.md §3`, verbatim incl. the "Deffered" typo) | Sales reps + phone/email automation. **This is the MASTER system of record (SOR-1, RULED 2026-07-18); the app mirrors it read-only and NEVER writes a funnel status.** |
...
> **RAHUL RULING — SOR-1 (RULED 2026-07-18): TeleCRM is the MASTER system of record; the app is a READ-ONLY MIRROR of the funnel.** *(supersedes Open Q1, `01-PRD.md §8.2`)* `🔴 Tier 1` (auth + reconciliation reads)
> The sales team creates every funnel event — **including acceptance** — in TeleCRM. The app **READS/mirrors** those events and reacts to them (e.g. detecting the flip to `accepted` fires the in-app acceptance experience). **The app NEVER writes a funnel status.**
> *(⚠️ Margin note — SOR-1 ruling 2026-07-18: this **REVERSES** the earlier "recommended default" in which the app wrote the states it controlled AND the carve-out in which `accepted` was "the one app-WRITTEN status." Both are deleted. The `is_admin()`-gated state-transition RPC that this block previously named as the writer of `accepted`/`waitlisted`/`rejected` — **SEC-DECISION-1 — is REMOVED; do not build it.** `03-DATA-MODEL-ERD.md` §2/§4.2 and `04-INTEGRATION-CONTRACTS.md` §9 must drop the same RPC. MEMBER-1 stays intact — the app still READS `accepted` to gate the veil, it just no longer writes it.)*
> **What this means per state.** The interview/decision states (`interview_scheduled`, `interview_done`, `accepted`, `waitlisted`, `rejected`) are all **`[reconciled]`** — mirrored from TeleCRM (± Calendly) via REQ-RECON-1. The app still writes only the two things that are genuinely app-internal and are **not** funnel statuses: (a) the auto-provisioned `auth.users` row on webhook (identity, not a funnel status), and (b) room **membership** rows once the mirrored state warrants them (MEMBER-1, §3.4). Payment advances are **mirrored from the existing Razorpay chain** (INTEG-PAY-1), not written by an app-owned order path for the ₹400. INTEG-CRM-1 read-only holds — there is no TeleCRM write-back in v1.
> **Acceptance-signal dependency (spec this — it is now load-bearing).** Because §1 mismatch 1 says TeleCRM carries no literal `Accepted` picklist value, the acceptance experience depends on the TeleCRM `accepted` state reaching the app **promptly**. Source it via the reconciler read path AND/OR a TeleCRM webhook if one is available (`04-INTEGRATION-CONTRACTS.md` §5.2/§9). This is the one integration seam SOR-1 adds a hard requirement to.
...
stateDiagram-v2
    ...
    state "APPLICATION (auth user exists — applicant)" as APP {
        ...
        interview_done --> accepted : TeleCRM flips to accepted (mirrored — SOR-1)
        interview_done --> waitlisted : TeleCRM mirrored decision
        interview_done --> rejected : TeleCRM Lost (mirrored)
        waitlisted --> accepted : seat released in TeleCRM (mirrored; manual v1)
...
| `accepted` | `accepted` | *(TeleCRM's `accepted` state, sales-created — must reach the app via reconciler read and/or webhook; §1 mismatch 1 + SOR-1)* | Sealed decision 6A → reveal → card 6C → claim 6E (**confirm CTA present**) | **E2** Veil-only (no membership) | `[reconciled]` — mirrored from TeleCRM's `accepted` flip (SOR-1, 2026-07-18); the app READS it to gate the veil, **never writes it**. *(Margin note — SOR-1 ruling: the prior `[written]` "one carve-out: app admin decision at Stage 06" is DELETED.)* |
| `waitlisted` | `waitlisted` | *(TeleCRM mirrored decision — no explicit picklist value; derived on the reconciler)* | Kind letter (same staging, quieter, 6A path) | **E1** Applicant | `[reconciled]` — mirrored from TeleCRM; app never writes it (SOR-1) |
| `rejected` | `rejected` | `Lost` (mirrored) | Kind letter (6A path) | **E1** → terminal | `[reconciled]` — mirrored from TeleCRM `Lost`; app never writes it (SOR-1) |
| `accepted_not_confirmed` *(abandon)* | `accepted` | *(implicit)* | Claim screen 6E with held-seat countdown (persisted on scroll) | **E2** Veil-only (no membership) | `[reconciled]` — recovered by honest countdown copy (`REQ-DEC-5`, SEAT-1) |
...
Under the **MEMBER-1 ruling** (RULED 2026-07-18; §3.4; canonical statement in `03-DATA-MODEL-ERD.md` §4.6a, matched in `05-ACCESS-SECURITY.md` §4/§5.4): `accepted` grants **no** membership row and **no** read path into any real room-content table — a **locked-future veil only, built from public-safe / marketing-class data (the same class the public offering page exposes), never real room rows**; `confirmation_paid` grants a **scoped, heavily-redacted `pre_member`** row — the "veil lifts" into the whitelisted pre_start subset (flows 7B/7C, PRD REQ-DEC-5/REQ-LOCK-2) — and `enrolled` upgrades it to full `member`. This supersedes `ROOMS-ARCHITECTURE.md R-D2`'s stricter "enrolled-only" line on the membership-threshold point (R-D2 is lower in doc precedence than the PRD/flows; the override is flagged for Rahul in MEMBER-1). The seat-release story is preserved because a `pre_member` is scoped to the pre_member whitelist and a balance-due dwell is recoverable (B6), never a locked door. *(Margin note — MEMBER-1 ruling 2026-07-18: the prior "redacted preview RPC for `accepted`" stance was deleted here; `accepted` now holds zero room-content read grant.)*
...
> **RAHUL RULING — MEMBER-1 (RULED 2026-07-18): At which status is a `cohort_room_members` row written — and how does the accepted "locked future" render without membership?** `🔴 Tier 1 (access boundary)`
...
> **RULED (2026-07-18) — the authoritative three-tier access boundary. Apply it exactly; do not reinterpret.** *(Margin note: this replaces the council's prior "recommended default + conservative alternative" framing, and DELETES the previously-proposed redacted SECURITY-DEFINER preview RPC for `accepted`.)*
> - **`accepted`** (post-interview, confirmation-fee unpaid): **NO `cohort_room_members` row and NO read path into ANY real room-content table** — zero RLS grants into room content. The one room-adjacent thing shown is the **LOCKED FUTURE VIEW veil** on the confirm-seat screen (6E/7A), built **entirely from public-safe / marketing-class data — the same class the public offering page already exposes (the `offerings` row: title, poster, publicly-advertised faculty names, static module-name lock chips), never real room rows.** The confirm CTA is present. *The previously-proposed redacted SECURITY-DEFINER preview RPC (`get_cohort_room_preview`) into room content for `accepted` is **DELETED** — do not build it.*
> - **`confirmation_paid`**: a **scoped `pre_member` `cohort_room_members` row** is written by the resolver — `role='pre_member'` upserted on the `confirmation_payment_id` stamp via an exception-guarded AFTER trigger (same swallow-with-WARNING safety as T9). They enter the room as a **heavily-redacted LIMITED member.** **Recommended redaction whitelist (a RAHUL DECISION to tune — stated as the flagged default, matching `05` §4/§5.4):** pre_member **CAN** see the room masthead/theme, a this-week **OVERVIEW** (session titles + dates only), cohort-mate presence/count, the welcome/announcements channel (**READ only**), and the upcoming-session schedule/calendar; pre_member **CANNOT** see full curriculum detail, any recordings, assignment content or submission, any feedback, or mentor/instructor materials, and **CANNOT post/write** in community (read-only). Community write and full content unlock at `enrolled`. `pre_member` is its **own** access level — `member` (full member) is **NOT** widened to include it.
> - **`enrolled`** (remaining/full fee): full **`member`** — nothing redacted, full read + community write (the row is upgraded to `role='member'` at `enrolled`, T9/T10).
>
> `NFR-SEC-1` (membership server-derived, never client-claimed) still holds — the `pre_member` row is written by the resolver, not the client. **Rationale (recorded):** smaller attack surface (`accepted` users hold **zero** RLS grants into room content) and it matches Rahul's flow-feedback "locked future view → excitement to confirm unlocks it," extended to a two-stage ladder (locked veil → confirm → scoped-redacted `pre_member` → remaining fee → full `member`). The public admission page's own whitelisted anon-read path (SEC-PUBLIC-1) is **separate and unchanged**, as is the offering-page public data path. This supersedes `ROOMS-ARCHITECTURE.md R-D2` on the membership-threshold point (flagged for Rahul). **Per CLAUDE.md Tier-1: this stays council + adversarial-suite-green-on-shadow + Rahul's written sign-off before any RLS is applied; built behind the room feature flag until then. No code, no builds, no commits pending sign-off.**

---

## 4. The abandon branches (the top-of-funnel thesis, drawn precisely)

81% of applicants abandon, and ~69% of abandoners are contactable (`TALLY-UX-ANALYSIS.md §3/§4`). Recovering them is REQ-RECON-1 + the reminder ladder. There are **six** abandon/dwell branches. Each is a *dwell condition on a live state* (not a separate enum value), detected by reconciliation + a time threshold, and each has one named recovery mechanism and one silence condition. (B1–B4 are the pre-payment/decision drops the automated ladder fully owns; B5 is the deadline roll-forward; **B6 is the balance-due dwell** that appears because the MEMBER-1 ruling places a `pre_member` in the room at `confirmation_paid` with balance still owed — flows 7C's "Balance slips · reminder ladder, then a human call · never a locked door mid-class.")
...
> **Note:** B6 exists because the MEMBER-1 ruling places a scoped `pre_member` in the room at `confirmation_paid` (balance still owed) — a "never a locked door" in-room dwell. The recovery is the "pay your balance" ladder rung escalating to a human call (7C).
...
| T3 | `submitted → app_fee_paid` | ₹400 captured in the **EXISTING Tally-form Razorpay link** (INTEG-PAY-1) | ⚠️ **INTEG-PAY-1 (RULED 2026-07-18): the app does NOT route ₹400 through its own order path in v1 — the intake chain (Tally → in-form Razorpay ₹400 → Calendly) is UNCHANGED and app-external.** The reconciler NET-matches the existing/legacy Razorpay ₹400 back to the app user by **phone (INTEG-KEY-1, phone primary / email fallback)**, then mirrors → `app_fee_paid`; TeleCRM `Application Fee Paid` is the CRM mirror. *(Margin note — INTEG-PAY-1 ruling: the earlier "App path — `verify-razorpay-payment` stamps app_fee, the INTENDED path" is **PARKED for v1**; an app-owned ₹400 checkout is a deferred fast-follow needing Tally changes Rahul has deferred — do not delete the idea.)* | 🔴 Tier 1 (payments — reconciliation match) | existing Razorpay link + REQ-RECON-1 `[reconciled]` |
...
| T6a | `interview_done → accepted` | **TeleCRM flips the lead to accepted** (sales-created event) — mirrored, NOT app-written | Status mirrored to `accepted` from the TeleCRM `accepted` state reaching the app via the reconciler read path and/or a TeleCRM webhook (`04-INTEGRATION-CONTRACTS.md` §5.2/§9). ⚠️ **SOR-1 (2026-07-18): the app NEVER writes this status; the prior `is_admin()`-gated decision RPC (SEC-DECISION-1) is REMOVED — do not build it.** `Converted` still maps to `confirmation_paid`/`enrolled` (POST-confirmation), never to pre-confirmation `accepted`, so the accepted signal must come from TeleCRM's own `accepted` state, not from `Converted`. **Notification says "decision ready" and never carries the verdict** (`REQ-DEC-1`) | 🔴 Tier 1 (reconciliation read path / TeleCRM webhook) | TeleCRM (external) `[reconciled]` |
| T6b | `interview_done → waitlisted` | TeleCRM mirrored decision | Status `waitlisted` mirrored from TeleCRM; same reveal staging, quieter; app never writes it (SOR-1) | 🟡 | TeleCRM (external) `[reconciled]` |
| T6c | `interview_done → rejected` | TeleCRM `Lost` (mirrored) | Status `rejected` mirrored from TeleCRM `Lost`; kind letter; app never writes it (SOR-1) | 🟡 | TeleCRM (external) `[reconciled]` |
| T6d | `waitlisted → accepted` | Seat released to the waitlist in TeleCRM (**manual v1**, SEAT-1) | Sales/TeleCRM action mirrored to the app; automated release + waitlist cron is **fast-follow**, not v1; app never writes the status (SOR-1) | 🔴 Tier 1 (if automated) | TeleCRM (external) `[reconciled]` |
...
| T7u | *(side-effect of T7)* designed unlock | `confirmation_paid` reached | Unlock animation fires **exactly once** per confirmation, ≤900ms, transform/opacity only; reduced-motion = instant swap (`REQ-LOCK-2`). **Under the MEMBER-1 ruling this is the "veil lifts": the public-safe locked-future veil is replaced by the real scoped `pre_member` room**, backed by T7m below. | 🟡 | app (target) |
| T7m | *(side-effect of T7)* pre_member membership | `confirmation_paid` reached (MEMBER-1 ruling) | AFTER trigger on the `confirmation_payment_id` staged transition upserts `cohort_room_members(role='pre_member', source='derived')` scoped to the pre_member whitelist; **trigger failure must NOT block the payment write** (swallow with WARNING, same rule as T9). Retracted if the seat lapses. This is the **sole** membership grant at `confirmation_paid`; `accepted` never reaches it (veil-only, no membership row). | 🔴 Tier 1 (RLS/membership) | resolver `[written]` (target) |
...
| **E2** Veil-only | `accepted`, `accepted_not_confirmed` | **NOT membership, and NO room-content read path** — the veil renders from **public-safe / marketing-class data only** (the same class the public offering page exposes: the `offerings` row + static chrome); *no `cohort_room_members` row, and NO SECURITY-DEFINER preview RPC into room content — that path is **DELETED** (MEMBER-1 ruling 2026-07-18)* | The **LOCKED FUTURE VIEW veil** on the confirm-seat screen: public-safe offering/marketing framing (title, poster, publicly-advertised faculty names, static module-name lock chips) + the confirm CTA and held-seat countdown | **Any real room row** — room-content tables, `cohort_weeks`/`live_sessions`, the member-gated `cohort_room_configs` row, roster/PII, post/reply bodies, recording/Zoom URLs, faculty materials, fees, funnel state, any other applicant, every other cohort. `accepted` holds **zero** RLS grants into room content — the veil never sources a content/week/config row |
| **E3** Pre-member lobby | `confirmation_paid`, `balance_paid` (pre-enrol) | **`cohort_room_members` row with `role='pre_member'`** (MEMBER-1 ruling), read via `cohort_room_can_access()` scoped to the pre_member whitelist — **its own access level; `member` is NOT widened to include it** | **Recommended whitelist (a RAHUL DECISION to tune — flagged default, matches `05` §4/§5.4):** room masthead/theme, this-week **OVERVIEW** (session titles + dates only), cohort-mate presence/count, welcome/announcements channel (**READ only**), upcoming-session schedule/calendar — plus their own payment/enrollment records (own-account, not room content) | Full curriculum detail, any recordings, assignment content/submission, any feedback, mentor/instructor materials, the full commons, roster PII beyond cohort-mate presence/count; **community write (read-only)** — all unlock at `enrolled`; any other room |
| **E4** Member | `enrolled` / `room_pre_start` / `room_live` / `room_wrap` | `cohort_room_members` — the **one** table every room policy reads, via `cohort_room_can_access()` (`NFR-SEC-2`) | The full room for *their* `(offering,batch)`: today/weeks, sessions, assignments, recordings, record, commons, **roster — BATCH-scoped (ROSTER-SCOPE-1, confirmed 2026-07-18: a student sees only their own batch's cohort-mates, never the whole offering)** | **Any other room** (cross-room read = 0 rows; the LEAK_CANARY suite, `NFR-SEC-4`); **any other batch's roster within the same offering** |
...
**The E1→E2→E3→E4 boundary is the single most security-load-bearing line in the machine** and the reason MEMBER-1 exists. Under the MEMBER-1 ruling the boundary is: an `accepted` person is **not** a member and holds **zero RLS grants into room content** (their "locked future" is a veil rendered from public-safe / marketing-class data only — never a `cohort_room_members` row and never a room-content RPC; the earlier preview-RPC path is DELETED); a `confirmation_paid`/`balance_paid` person **is** a scoped `pre_member` (pre_member whitelist only, read-only community, written by the resolver never the client, `member` never widened to include it); a `member`'s full read + community write begins at `enrolled`. Every access transition (E-anything) is `🔴 Tier 1` and gated by the adversarial access suite (`qa-harness/cohort-room-access.spec.mjs`, `NFR-SEC-4`) — which must assert that a `pre_member` reads only the whitelist and **zero** full-member content (no recordings, no feedback, no community write), and that an `accepted` user has **no read path into any room-content table at all** (the suite asserts `cohort_announcements`/`cohort_room_posts`/`cohort_resources`/`cohort_demo_entries`/`cohort_weeks`/`cohort_room_configs` return **zero rows** for an `accepted` fixture; the veil renders from the `offerings` row + static chrome alone).
...
sequenceDiagram
    ...
    Note over U,V: PATH A — app-owned staged checkout — ⚠️ RETIRED for v1 (INTEG-PAY-1): app does NOT route ₹400
    ...
    Note over U,R: PATH B — existing in-form Razorpay ₹400 link — the v1 path (INTEG-PAY-1); reconciler NET-matches by phone
...
**Why both paths are drawn.** Path A is what the code supports; Path B is what production actually does (`FUNNEL-DATA-AUDIT.md §2`). *(Margin note — INTEG-PAY-1 ruling 2026-07-18: Path A, the app-owned ₹400 order path, is **RETIRED for v1**. The existing Tally → in-form-Razorpay → Calendly intake chain is **unchanged and app-external** — the app inserts nothing into it and does not route the ₹400. **Path B — the reconciler NET-matching the existing/legacy Razorpay ₹400 to the app user by phone (INTEG-KEY-1) — IS the v1 path.** Path A returns only as a deferred fast-follow that needs Tally changes Rahul has parked.)* REQ-RECON-1 is the bridge that lets the state machine advance to `app_fee_paid` even when the money came through the existing in-form link — and it must **surface the join-completeness / orphan rate as a health alert** rather than silently under-counting (`REQ-RECON-1` acceptance, Risk R8). The success screen's three-slot booking (step "Success screen 5A") is the guard that prevents the `fee_paid_no_schedule` (B3) dwell.

---

## 8. Riskiest handoff (b): acceptance → confirmation fee → enrolment

This seam turns an applicant into a room member. It crosses the E1→E2→E3→E4 access boundary (§6) — writing a scoped `pre_member` at `confirmation_paid` and a full `member` at `enrolled` into `cohort_room_members`, the one table all room RLS reads. Guard integrity here is where a cross-room leak or a "paid but no room" dead-end would be born.

```mermaid
sequenceDiagram
    autonumber
    participant AD as TeleCRM mirror (decision) + Admin (enrol)
    participant U as Applicant
    ...
    AD->>DB: mirror TeleCRM 'accepted' → status='accepted'  [guard: SOR-1 — TeleCRM-sourced mirror; app NEVER writes it; SEC-DECISION-1 RPC removed]
    ...
    V->>AS: fire designed unlock 7B (≤900ms, once)  [NOTE: veil lifts → now a scoped pre_member via T7m — MEMBER-1]
    opt balance step
...
For every `[reconciled]` state in §3, REQ-RECON-1 derives it from the three external systems keyed on the logged-in user's **phone (primary) then email (fallback), recording which resolved** (INTEG-KEY-1) — the same join the whole funnel already runs on (`FUNNEL-DATA-AUDIT.md §6`). This is the referee that lets the machine advance without an app writer for the interview/decision states — and under SOR-1 (2026-07-18) it is now the **sole** source of the `accepted` state as well (TeleCRM-mirrored, never app-written).
...
| TeleCRM `accepted` state (sales-created; via reconciler read and/or webhook — SOR-1) | `accepted` | See your decision (fires the acceptance experience + veil) |
...
| **07** Locked future — 7A/7B/7C | `accepted` (**E2 veil-only**, no membership, public-safe data) → 7B "veil lifts" at `confirmation_paid` → 7C scoped pre_start lobby (**E3 `pre_member`**); `enrolled`/`room_pre_start` induction (**E4 member**) | REQ-LOCK-1..3, MEMBER-1 |
...
- **T7 / T8 — staged-payment advances** for the ₹8k confirmation + balance (payments) — and the sacred `isIOS()` guard (`NFR-SEC-5`, do-not-touch). **T3 (the ₹400) is now a reconciliation MATCH, not an app order path** (INTEG-PAY-1, 2026-07-18): the app-external in-form Razorpay ₹400 is NET-matched by phone, not routed through the app.
- **T6a — the acceptance mirror seam (SOR-1, 2026-07-18)** — TeleCRM is the master SOR; `accepted` is mirrored, **never app-written**, and the prior `is_admin()` decision RPC (SEC-DECISION-1) is **removed — do not build it**. The load-bearing net-new work is getting TeleCRM's `accepted` state to the app promptly via the reconciler read path and/or a TeleCRM webhook (`04-INTEGRATION-CONTRACTS.md` §5.2/§9); this gates the veil (MEMBER-1) so its latency is a Tier-1 concern.
- **T4 — the Calendly webhook receiver** (net-new external→app infra + signature verification + `interview_modality` column; none exists today). Calendly stays inside the existing external intake chain in v1 (INTEG-PAY-1 / INTEG-CAL-1 single account); in-app embedding is a nice-to-have, not required.
- **T9 / T10 — enrolment → `cohort_room_members` resolver trigger** (RLS/membership); AFTER-trigger failure must be non-blocking.
- **The E3 `pre_member` resolver trigger (MEMBER-1 ruling, T7m)** — writes a scoped-redacted membership row at `confirmation_paid`; adversarial-suite-gated. **Note the deleted path:** the previously-proposed E2 redacted preview RPC into room content for `accepted` is **removed** — `accepted` is veil-only (public-safe / marketing-class data, **zero** RLS grants into room content), so there is no `accepted` room-content access path to build or gate. Built behind the room flag until Rahul's written sign-off.
...
1. **SOR-1 (§1) — RULED 2026-07-18 (no longer open):** TeleCRM is the **master SOR**; the app is a **read-only mirror** and **never writes a funnel status**. All interview/decision states (incl. `accepted`) are `[reconciled]` from TeleCRM (± Calendly). The prior admin decision RPC (SEC-DECISION-1) is **removed**. The remaining integration work SOR-1 creates is the **acceptance-signal seam** — getting TeleCRM's `accepted` state to the app promptly via reconciler read and/or a TeleCRM webhook (tracked in `04-INTEGRATION-CONTRACTS.md` §5.2/§9), and the INTEG-PAY-1 reconciler NET-match of the app-external ₹400 (§T3, §7).
2. **MEMBER-1 (§3.4, 🔴 Tier 1) — RULED 2026-07-18:** the membership-grant threshold and the accepted "locked future" are fixed. **`accepted` = veil-only (public-safe / marketing-class data, no membership, no room-content read path — the redacted preview RPC is DELETED); `confirmation_paid` = scoped-redacted `pre_member`; `enrolled` = full `member`.** The pre_member redaction whitelist is the one remaining Rahul-tunable knob (stated as the flagged default in §3.4/§6). Still gated on council + adversarial-suite-green-on-shadow + Rahul's written sign-off before any RLS applies; built behind the room flag until then.
3. **No-show → decision routing (T4n):** does a `No show` auto-route toward `rejected` after N missed re-books, or stay open for a manual sales decision in TeleCRM (mirrored to the app per SOR-1, never app-written)? *Recommended default:* stay open (manual, TeleCRM-side), since no-show is a guardrail the team watches, not an auto-reject; confirm with Rahul.
