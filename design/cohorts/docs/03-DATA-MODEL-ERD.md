...
**The design consequence** (PRD Open Q1, RULED 2026-07-18 by SOR-1 — see the box below): the app **reads/reconciles/mirrors** the external systems by phone/email (REQ-RECON-1) and **owns authoritatively only what it controls** — provisioning (identity spine), staged payments already flowing through the app, and the whole room backbone. The reconciled/mirrored funnel stage (including `accepted`) is written to *additive* columns (§4.2), never by overwriting the sacred staged-payment path and never as a `status` funnel transition. *Margin note 2026-07-18: SOR-1 settles Open Q1 — the app does NOT graduate to writer of the interview/accept/reject states; TeleCRM is the master and the app stays a read-only mirror. This model still supports the mirror without any schema change to the payment pipeline.*

> **RULED — DATA-SoT-1 / SOR-1 (Rahul, 2026-07-18): TeleCRM is the MASTER system of record; the app is a READ-ONLY MIRROR. The app NEVER writes a funnel status — including `accepted`.**
> *Margin note 2026-07-18 — REVERSES the prior "recommended reconciler-default WITH an `accepted` app-write carve-out." The earlier stance made `accepted` "the one app-WRITTEN status" via an `is_admin()`-gated state-transition RPC (SEC-DECISION-1). SOR-1 REMOVES that RPC and that carve-out entirely.* The sales team creates every funnel event — **including acceptance** — in TeleCRM; the app **reads/mirrors** them and reacts. The reconciler writes only *derived/mirror* state to the *new additive columns* (`reconciled_stage`, `reconciled_at`, `reconciliation_source`, `recovery_marker` — §4.2), keyed to the reconciled identity (phone-primary, email-fallback — INTEG-KEY-1). The app authoritatively writes **only** provisioning (identity spine), staged payments it already processes, and the room backbone — **never** a `cohort_applications.status` funnel transition (interview stages, `accepted`, `rejected`, `waitlisted`, and the payment stages are all a TeleCRM/Razorpay/reconciler fact the app mirrors, not authors). **`accepted` is now TeleCRM-sourced:** because the sales team marks acceptance in TeleCRM, the mirror MUST receive the flip to `accepted` **promptly** — via the reconciler read path and/or a TeleCRM `accepted` webhook if available (§4.2 note, §4.3 TeleCRM map). Detecting that flip (not writing it) is what fires the in-app acceptance experience and the decision ceremony (COPY CD-06-REV-02/CD-06-LTR renders a mirrored verdict, not an app-authored one). MEMBER-1 is unaffected — the app still **READS** `accepted` to gate the LOCKED FUTURE VIEW veil; it just never writes it. The read-only-TeleCRM contract (INTEG-CRM-1) is now total: no app write-back of any kind in v1. The payment pipeline stays byte-for-byte untouched (PRD §4.4). 🔴 Tier 1 (funnel status semantics; the mirror/reconciler read path is authorized in `05-ACCESS-SECURITY.md`).
...
| 27 | `cohort_transcripts` (view/RPC) | 🟥 NEW (computed) | Derived | REQ-FINISH-1 (**single Completion cert — STANDING-1, no honors tiers**) | §4.8 |
...
| `app_fee_inr` | numeric(10,2) | no (0) | 🟩 the ₹400 application fee — a **separate non-refundable review fee, NOT credited toward tuition** (FEE-1, 2026-07-18; amount→product map, `FUNNEL-DATA-AUDIT.md` §4). *Margin note: REVERSES the earlier fast-follow "credit ₹400 toward tuition" lean — no tuition-credit ledger column exists for it. Charged via the EXISTING in-Tally Razorpay link, not an app order (INTEG-PAY-1).* | App |
...
| `status` | text CHECK (11 values) | no (`submitted`) | 🟩 **the D2 state machine, held as a READ-ONLY MIRROR (SOR-1).** `submitted → app_fee_paid → interview_scheduled → interview_done → accepted → confirmation_paid → balance_paid → enrolled`; exits `rejected\|withdrawn\|waitlisted`. *Margin note 2026-07-18 (SOR-1): the app writes NO funnel transition here — including `accepted`; these values are mirrored from TeleCRM (master SoR) via the reconciler read path / TeleCRM webhook (§2 box, §4.3). No `is_admin()` decision RPC writes this column.* | TeleCRM→App (mirror) |
...
> **Note on `accepted` (SOR-1, 2026-07-18 — acceptance is TeleCRM-SOURCED):** *Margin note — REVERSES the prior "app owns `accepted` as an admin decision (Stage 06)" caveat.* Under SOR-1 the **sales team marks acceptance in TeleCRM** (the master SoR), so acceptance is now an **external fact the app mirrors**, not one it writes. The app receives the flip to `accepted` via the reconciler read path and/or a TeleCRM `accepted` webhook (INTEG-CRM-1 read-only; see §4.3 TeleCRM map, where an explicit "Accepted" state is now expected between `Interview completed` and `Converted`). It lands in `reconciled_stage` (and mirrors into `status`) — **the app writes no `accepted` transition itself.** ⚠️ Dependency: the acceptance experience only fires promptly if TeleCRM's `accepted` state reaches the app quickly; spec the reconciler cadence and/or a TeleCRM webhook accordingly (`04-INTEGRATION-CONTRACTS.md`). The old requirement that acceptance be app-written for the decision ceremony to exist is retired — the ceremony now renders a **mirrored** verdict.
...
| `Accepted` (sales-team-set) | **accepted** — TeleCRM-sourced per SOR-1 (2026-07-18); the app MIRRORS this, never writes it. Needs to reach the app promptly (reconciler cadence / TeleCRM webhook) to fire the acceptance experience + MEMBER-1 veil gate. *Margin note: previously treated as unreadable-from-TeleCRM and app-written; SOR-1 makes the sales team the author in TeleCRM.* |
...
**Razorpay** (SoT: captured payments). **Amount = product** — 0/199 recent payments carry an app id, so the join is **`contact` (phone) primary, `email` fallback** + amount (INTEG-KEY-1; `FUNNEL-DATA-AUDIT.md` §2/§4): ₹400 = app fee, ₹8k = confirmation, balance = the remainder. Reconciler reads captured amounts to confirm `app_fee_paid` / `confirmation_paid` / `balance_paid` independently of the dormant app order path. *Margin note 2026-07-18 (INTEG-PAY-1): the ₹400 review fee is charged by the **EXISTING in-Tally Razorpay link** — the app inserts **no `payment_orders` row** for the ₹400 and does **not** route it through its own order path. The reconciler simply **NET-matches** those existing/legacy ₹400 captures back to app users by phone (email fallback). The ₹400 is a separate **non-refundable review fee, NOT credited toward tuition** (FEE-1) — no tuition-credit / deposit-reframe ledger field exists for it anywhere in this schema.*
...
**§4.6a — The membership boundary (three tiers: `accepted` → `pre_member` → `member`) — the single most security-load-bearing line in the room. 🔴 Tier 1.**

> **RULED — MEMBER-1 (Rahul, 2026-07-18): the three-tier cohort-room access boundary.** `🔴 Tier 1 (access boundary)`
> *Margin note 2026-07-18: this **supersedes** the prior "RAHUL DECISION — a real pre_start membership vs a sharpened preview" framing. The council's previously-proposed redacted SECURITY-DEFINER **preview RPC for `accepted`** is **deleted** — see §4.7a and the §7 matrix, both corrected below. No "redacted preview into room content" exists for `accepted` anywhere in this schema.* Stated once here; referenced identically by `01-PRD.md` (REQ-LOCK-1/2, REQ-DEC-5) and `02-STATE-MACHINE.md` (§3.3, §6, T7u).
>
> **The ladder is three tiers, and only two of them hold a `cohort_room_members` row:**
> 1. **`accepted`** (post-interview, offer made, confirmation fee NOT yet paid) → **NO `cohort_room_members` row, and NO read path into any real room-content table.** There is no preview RPC into room content for `accepted`. The only room-adjacent surface is the **LOCKED FUTURE VIEW veil** on the confirm-seat screen — a teaser built from **public-safe / marketing-class data** (the same class the public offering page already exposes via the SEC-PUBLIC-1 anon-read path), **never** real room rows. The confirm CTA is present. Data consequence: `accepted` holds **zero RLS grants** into room content — the smallest possible attack surface.
> 2. **`confirmation_paid`** → the resolver writes a scoped **`role='pre_member'`** row (write rules below). The `pre_member` **does enter the room** as a *limited* member under **heavy redaction enforced at the RLS/RPC layer (`05-ACCESS-SECURITY.md`), not in this schema** (see the redaction note below). A `pre_member` with balance still due is a recoverable dwell (STATE §4 branch B6), never a locked door mid-class.
> 3. **`enrolled`** (remaining/full fee + admin enrol) → the row is **upgraded to `role='member'`** — full read + community write, nothing redacted.
>
> **Rationale (recorded):** smaller attack surface (`accepted` users hold zero RLS grants into room content) and it matches Rahul's flow feedback "locked future view → excitement to confirm unlocks it," extended to a two-stage ladder (locked teaser → confirm → scoped-redacted → remaining → full). The public admission page's own whitelisted anon-read path (SEC-PUBLIC-1) is **separate and unchanged**; the offering-page public data path is unchanged.
> **Still gated:** built behind the room feature flag; council + adversarial-suite-green-on-shadow + Rahul written sign-off before any RLS is applied (CLAUDE.md Tier 1).

**Membership resolver (§4.6a) — the write rules this ruling fixes:**
- **`accepted` writes NO membership row.** The resolver does nothing on acceptance; there is no `cohort_room_members` row until the confirmation fee is captured.
- **`confirmation_paid` writes the `pre_member` row.** The resolver writes a `role='pre_member'`, `source='derived'` row on the `confirmation_payment_id` stamp — an exception-guarded AFTER trigger on the staged-payment status transition, same swallow-with-WARNING safety as the enrolment trigger. Retracted if the seat lapses.
- **`enrolled` upgrades to `member`.** The same resolver upgrades the row to `role='member'` when the enrolment truth (`enrolments.status='active'`) appears.

So `cohort_room_members.role` carries the two **applicant-tier** values this ladder needs — **`{pre_member, member}`, with `accepted` deliberately ABSENT** (it holds no row at all) — alongside the separate staff/lifecycle roles (`mentor`, `host`, `alumni`) already in the CHECK. **What each tier may see is not a schema fact.** The `pre_member` redaction whitelist — RECOMMENDED default, a **RAHUL-tunable decision** (masthead/theme + a this-week overview of session titles/dates + cohort-mate presence/count + read-only welcome/announcements + the upcoming-session schedule; and NOT full curriculum detail, recordings, assignment content/submission, feedback, or mentor/instructor materials, with community **read-only**) — is **enforced at the RLS/RPC layer in `05-ACCESS-SECURITY.md`, not in this table.** This schema records only *that* the row exists and its `role`; the redaction lives in 05.
...
### 4.7a Room RPCs — the net-new access-load-bearing write functions (🔴 Tier 1)

The room's read/write surface is not plain RLS on the tables alone: the **channel/mentor-answer write validation** cannot be expressed as a table policy and must live in a SECURITY DEFINER RPC. (The previously-proposed redacted **preview RPC for `accepted`** is **deleted** — MEMBER-1, 2026-07-18; see (1) below.) The write RPCs are cross-linked from `02-STATE-MACHINE.md` §11 and authorized in `05-ACCESS-SECURITY.md` §5.3/§5.4; the crew must not build room access from the table policies alone or it will drop these controls.

**(1) ~~`get_cohort_room_preview(p_offering uuid)` — the E2/E3-preview RPC~~ — DELETED per MEMBER-1 (Rahul, 2026-07-18). 🔴 Tier 1.**
> *Margin note 2026-07-18: MEMBER-1 removed the redacted SECURITY DEFINER preview RPC for `accepted`.* `accepted` now holds **no read path into any real room-content table** — there is no preview RPC. The confirm-seat **LOCKED FUTURE VIEW veil** shown to `accepted` is rendered by the front end from **public-safe / marketing-class data** (the same class the public offering page already exposes via SEC-PUBLIC-1), **never** from a room-content RPC and never from real room rows. `confirmation_paid` users are **`pre_member`s** who read the room through the **normal member path** under **heavy redaction enforced at the RLS/RPC layer (`05-ACCESS-SECURITY.md` §5.3/§5.4)** — there is no separate preview RPC for them either. This doc records only the membership row (§4.6a); the redaction whitelist and its enforcement live in 05.
...
**`cohort_transcripts`** 🟥 **NEW (computed — a view/RPC, not a stored table):** the transcript is *derived* server-side from `cohort_week_attendance` + `cohort_week_submissions` — **not stored**, so it recomputes live from week 1 and cannot drift. RPC signature: `get_cohort_transcript(p_user uuid, p_offering uuid) → { attendance_pct, weeks[], recovered_count, submissions[], mentor_grade, is_complete }`. *Margin note 2026-07-18 (STANDING-1) — REVERSES the CRO-3 honors-tier design.* **There are NO honors tiers in v1 and NO Distinction/Merit/provisional-cutoff fields.** The certificate is a **single "Completion" certificate issued on cohort completion.** The RPC therefore returns a boolean `is_complete` (completion computed only), **not** a `standing_tier` enum — the old Distinction ≥90% / Merit ≥85% cutoff logic is deleted. **Attendance is still TRACKED and SHOWN** (`attendance_pct`, `recovered_count`, in-room per-week; recovery via recordings stays) but it does **NOT define certificate tiers**. Whether attendance gates the Completion cert at all is a minor Rahul decision (default per STANDING-1: **issued on completion, no hard attendance gate in v1**; the 85% `attendance_threshold_pct` remains available as an optional eligibility gate, not a tier boundary). SoT: Derived.

> **RULED — STANDING-STORE-1 / STANDING-1 (Rahul, 2026-07-18): single Completion certificate, computed live; NO honors tiers.**
> *Margin note 2026-07-18 — REVERSES the earlier "snapshot the final tier" framing, which assumed Distinction/Merit tiers.* There is **no standing tier to store** — v1 issues one **Completion** certificate. Compute completion **live** (a view/RPC) so the transcript is honest week-to-week and needs zero funnel write path; snapshot only the immutable completion fact (and the attendance/submission figures as-of issue) into `certificates.metadata` at issue time for the artifact. **Do NOT persist a `standing_tier`** — that field and its cutoffs are removed. Attendance remains tracked/shown but non-tiering. Alternative (perf only): a materialized `cohort_completion` table refreshed nightly if live computation misses p95<150ms at 200-member scale — measure first. 🟡 Tier 2.
...
| batch ↔ enrolment (roster) | **M : N** | `cohort_batch_members` join table | 🟩 the delivery-roster M:N. **BATCH-scoped (ROSTER-SCOPE-1):** a student's roster / cohort-mate view resolves through `cohort_batch_members.batch_id = <viewer's batch>`, **never** offering-wide — see §6.1 predicate. |
...
**Roster / cohort-mate scope is BATCH-scoped, not offering-scoped (ROSTER-SCOPE-1, confirmed 2026-07-18).** A student sees only their **own batch's** cohort-mates, never every member of the offering. The canonical predicate for "who are my cohort-mates" is: the set of `cohort_room_members` (or `cohort_batch_members`→`enrolments`) rows sharing the **viewer's `batch_id`** — i.e. `WHERE crm.offering_id = <offering> AND crm.batch_id = <viewer's batch_id> AND crm.role IN ('member','pre_member','alumni')`. Offering-wide roles (`mentor`/`host`, whose `batch_id IS NULL`) are the only rows that span batches, and they read across batches by design; a plain member's roster read must **never** drop the `batch_id` equality. This is enforced at the RLS/RPC layer (`05-ACCESS-SECURITY.md`); the schema supports it because both `cohort_room_members` and `cohort_batch_members` carry `batch_id`. The MEMBER-1 "cohort-mate presence/count" surface (§4.6a) is computed under this same batch predicate.
...
| `accepted` | `status='accepted'` / `reconciled_stage='accepted'` — **MIRRORED from TeleCRM** (SOR-1, 2026-07-18; sales-team-set in TeleCRM, app never writes it — §2 box, §4.2 note, §4.3 map) | 🟩 (read-only mirror; TeleCRM SoR) |
...
| **Accepted — LOCKED FUTURE VIEW veil** (no membership row, no room-content read) | **No `cohort_room_members` row and no read path into any real room-content table** (MEMBER-1, 2026-07-18). The confirm-seat veil is rendered from **public-safe / marketing-class data** (SEC-PUBLIC-1 class), **not** a room-content RPC and not real room rows. The prior `get_cohort_room_preview` RPC is deleted (§4.7a). | 🟥 (front-end veil; 🔴 boundary in 05) |
| **Membership: `pre_member`** (confirmation_paid → scoped room member) | `cohort_room_members.role='pre_member'`, written by the resolver on the `confirmation_payment_id` stamp (MEMBER-1, 2026-07-18). Enters the room under **heavy redaction enforced at the RLS/RPC layer (05), not in this schema** — RAHUL-tunable default whitelist: masthead/theme, this-week overview (titles+dates), cohort-mate presence/count, read-only welcome/announcements, upcoming-session schedule; NOT curriculum detail/recordings/assignment content/feedback/mentor materials, community read-only. Upgraded to `member` at `enrolled`. | 🟦 (role exists; resolver trigger, §4.6a) |
...
| `cohort_room_post_write()`/`cohort_room_reply_write()` (channel + is_mentor_answer validation; REVOKE direct INSERT) | RPC (§4.7a) | 🔴 (access boundary) | LOCK-1, COMM-1/2 |
| `pre_member` resolver trigger on `confirmation_payment_id`; `accepted` writes NO row; upgrade to `member` at `enrolled` (MEMBER-1, 2026-07-18; behind room flag) | resolver / `cohort_room_members` | 🔴 (access boundary) | LOCK-2, MEMBER-1 |
...
| `get_cohort_transcript()` (computed completion; **no `standing_tier`** per STANDING-1) | RPC | 🟡 | FINISH-1 |
...
1. **DATA-SoT-1 / SOR-1 (§2, 🔴 Tier 1) — RULED (Rahul, 2026-07-18), no longer open.** TeleCRM is the **master system of record**; the app is a **read-only mirror** and writes **no** funnel status. *Margin note — REVERSES the prior "settled carve-out: `accepted` is app-written (admin decision, Stage 06)."* Acceptance is now **TeleCRM-sourced** (sales-team-set); the app mirrors it via the reconciler read path / TeleCRM webhook, and the `is_admin()` decision RPC (SEC-DECISION-1) is removed. The additive-column design carries derived/mirror state only. Remaining crew task: spec the reconciler cadence / TeleCRM `accepted` webhook so the flip reaches the app promptly (MEMBER-1 veil + acceptance experience depend on it).
0. **MEMBER-1 (§4.6a, 🔴 Tier 1) — RULED (Rahul, 2026-07-18), no longer open.** Three-tier boundary: `accepted` writes NO membership row and gets NO room-content read (only the public-safe LOCKED FUTURE VIEW veil); `confirmation_paid` gets a scoped `pre_member` row that enters the room under heavy redaction enforced at the RLS/RPC layer (05); `enrolled` upgrades to `member`. The council's redacted preview RPC for `accepted` is deleted. Still built behind the room flag and gated on council + adversarial-suite-green-on-shadow + Rahul written sign-off before any RLS is applied.
