...
const DemoEntryCard = ({ entry, memberName, city, isOwn, onEdit }: DemoEntryCardProps) => {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface">
      <div className="relative isolate min-h-36 overflow-hidden border-b border-border bg-canvas px-5 py-6">
        <div
          aria-hidden="true"
        ...
        <h2 className="mt-4 line-clamp-3 font-serif text-2xl leading-tight text-foreground">
          {entry.title}
        </h2>
      </div>
          ...
          {entry.files.map((file) =>
            file.signedUrl ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="focus-ring pressable inline-flex min-h-11 items-center gap-2 rounded-full border border-border px-4 text-sm text-foreground"
              >
                <FileText size={15} strokeWidth={1.5} aria-hidden /> {file.name}
              </a>
            ) : null,
          )}
          {isOwn && onEdit && (
