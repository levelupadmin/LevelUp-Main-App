...
| 1 | Core loop: Home, button/card adoption, tab bar, shared-element, craft-bug sweep | `design/phase-1` | ✅ PROMOTED 2026-07-03: Android 613/3.5.0 → production @ 20% staged (ramp: play-publish.mjs --promote 613 --track production --rollout 1). Web prod live (merge 9dc6389). iOS TestFlight pending Xcode install. 3 gates + 3 sprints + council (REVISE→resolved). ~9.5M Opus tokens, ~120 agents. NOT merged to main (web-prod gate). Carry-overs in phase-2 brief: greeting band coarse-pointer parking (index.css:227, Tier-1+council), Learn empty CTA 40px@360, CatalogSection pills a11y, UpcomingSessions focus ring, MyCoursesPage .pressable, ken-burns device perf, HeroPlayChip legacy blur |
| 2 | Screening room: ChapterViewer, exit animations, skeleton crossfade | `design/phase-2` | ✅ PROMOTED 2026-07-07 on Rahul's order (web prod + Android 614/3.6.0 @ 100% + iOS build 18 TestFlight): build (7 tasks) → gate FIX-LIST(16) → sprint 15/15 → re-gate FIX-LIST(15) → sprint 15/15 → council SHIP-AFTER-VERIFICATION (0 blocking; Rahul's on-device pass = the verification; his checklist = council mustVerify). ~7.5M Opus tokens, ~120 agents. Deferred: Moments tab (product call), phase-2.1 root fix for coarse-pointer sticky (#root overflow-x:clip — council-designed, needs own device pass), reference-counted body-lock module, recap desktop-wheel containment, sonner pin to 1.7.4 |
| 3 | Money pages: PublicOffering/checkout choreography (+PostHog if approved) | `design/phase-3` | ✅ PROMOTED 2026-07-08: web + Android 615@15% (superseded by 616) + iOS 19 TestFlight. 3 gates (13→10→5) + 3 sprints + council REVISE (dust portal, IN-locked guest phone) → shipped. Enrichments: sticky↔inline handoff, pay-button arc, champagne dust. ~17M tokens |
| 4 | Tactility: sheets/gestures, stats/streak (if approved), Community/Login/Profile | `design/phase-4` | ✅ PROMOTED 2026-07-08: web + Android 616@15% (superseded by 617) + iOS 20 TestFlight. 2 gates (17→6) + 1 sprint + council REVISE (PTR-behind-sheets guard, true body-lock docs) → shipped. MorphSheet grammar, PTR v2, certificates+share, OTP choreography. Streak behind default-off flag (Rahul call pending). ~16M tokens |
| 5 | Hardening: device matrix, perf, microcopy → first production-promotion candidate | `design/phase-5` | ✅ PROMOTED 2026-07-08: web + Android 617/3.9.0@15% + iOS 21 TestFlight. CENTERPIECE: June-14-era coarse html/body overflow rule RETIRED (#root clip + iOS15 fallback + portal clips; native sticky app-wide; probe machinery deleted). 2 gates (15→10) + 2 sprints + council REVISE (shimmer keyframes were never emitted!, toast tab-bar offset, honest UPI copy). One toast/skeleton/EmptyState family; useBodyScrollLock module; axe 0×10 routes. ~15M tokens. Rahul REAL-DEVICE Android scroll pass = the 15%→100% gate |

Update the Status column as phases move (brief ready → building → gating → shipped-internal → promoted).

**Post-phase-5 pipeline (vision backlog):** Phase 6 "The Curtain" (cold start & data layer) → 7 (first act & brand moments) → 8 (re-engagement) → 9 (discovery/community surface) → 10 (platform polish). Community program (design/community/) runs as its own track once Rahul picks a direction. Standing kill-switch recommendation from the phase-5 council: remote-toggleable restore of the legacy coarse overflow rule (Capacitor binaries cannot be hot-fixed) — evaluate in phase 6.
